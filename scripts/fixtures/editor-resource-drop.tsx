...
import { EMPTY_MARKDOWN_LINK_COMMANDS, ExplorerTree, getEditorPanes, type DataPort, type DocumentDataNode, type MarkdownWorkspaceEnvironment } from "@puppyone/shared-ui";
...
function Harness() {
  ...
  useEffect(() => {
    const record = (event: DragEvent) => window.resourceSmoke.record({ event: event.type, types: [...(event.dataTransfer?.types ?? [])], files: [...(event.dataTransfer?.files ?? [])].map((file) => window.puppyoneDesktop.getPathForFile(file)) });
    window.addEventListener("drop", record, true);
    return () => window.removeEventListener("drop", record, true);
  }, []);
  const exportNodes = useResourceDragExport((resource) => ({ folder: { id: "smoke-root", workspace }, resourceUri: resource, providerPath: resource === file.path ? "docs/中文 file.md" : "existing.md" }),
  ...
  return <main style={{ display: "grid", gridTemplateColumns: "230px 1fr", width: "100vw", height: "100vh", background: "white", color: "#222" }}>
    ...
    <section style={{ position: "relative", minWidth: 0, minHeight: 0 }}>
      <DesktopEditorSplitView aiEditRequest={null} dataPort={port} editorGroup={editor.state} layout={editor.paneLayout}
        ...
        markdownEnvironment={environment} documentNavigation={{ openPath: () => {} }} workspace={workspace}
