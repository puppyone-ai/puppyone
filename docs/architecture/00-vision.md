...
PG 存什么（控制平面 — 不持有文件内容）：
├── projects.mut_version          ← 当前版本号
├── projects.mut_root_hash        ← 当前 Merkle tree 根 hash
├── mut_commits                   ← 版本历史（who, when, changes, root_hash）
├── audit_logs                    ← 审计日志
...
```
MUT handle_push 检测到 base_version < current_version
    │
...
```
handle_push(files, base_version, who, scope)
  │
  ...
  │     └─ 通过 → 继续
  │
  ├─ 2. 乐观锁检查：base_version == current_version ?
  │     │
  │     ├─ YES → 直接写入 Merkle tree，version++
  │     │        记录 mut_commit + audit_log
  │     │        return { status: "clean", version: N+1, merged: false }
  │     │
  │     └─ NO → 有人先改了，需要合并
  │              │
  │              ├─ 3. 取出 base tree (base_version 的 root_hash)
  │              │      取出 current tree (current_version 的 root_hash)
  ...
  │              │     └─ 二进制 → LWW 兜底
  │              │
  │              ├─ 全部自动合并成功 → 写入合并结果，version++
  │              │   return { status: "merged", version: N+1, merged: true }
  │              │
  │              └─ 有真冲突 → 写入可合并部分，冲突文件标记
  │                  return { status: "conflict", version: N+1, conflicts: [...] }
  │
  └─ 5. Post-commit：记录 mut_commit + audit_log + 触发 hooks
```
