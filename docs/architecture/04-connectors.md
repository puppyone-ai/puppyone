...
所有 connector 类型共享一张 `connections` 表（`provider` 字段区分类型），
但按**数据写入方式**分为两大阵营：
...
```
connectors/
├── manager/            统一 CRUD 入口（connections 表）
│
...
- **Access Key** — 每个 connection 的 `access_key` 字段用于认证（`cli_xxx` / `sbx_xxx`）
...
| `manager/router.py` PATCH update | 更新 connection trigger 时注册/移除 |
| `manager/router.py` DELETE connection | 删除 connection 时移除 |

### 3.2 Trigger 配置格式

存储在 `connections.trigger` JSON 字段：
...
├── repository.py      SyncRepository（connections 表访问）
├── router.py          API 路由
...
├── service.py         FilesystemService（连接生命周期管理）
...
| 端点 | 用途 |
|------|------|
| `POST /bootstrap` | 创建连接、分配 access_key |
| `GET /{id}/connection-status` | 查询连接状态 |
...
├── repository.py          db_connections 表访问
├── providers/
...
**已知架构问题**：使用独立的 `db_connections` 表而非统一的 `connections` 表，
是一个待修复的一致性问题。
...
```
POST   /connections        → 按 provider 路由到子服务创建
GET    /connections        → 按 project_id / provider / status 过滤
GET    /connections/{id}   → 查询详情
PATCH  /connections/{id}   → 更新 status / trigger / config
DELETE /connections/{id}   → 删除（同时移除 scheduler job）
POST   /connections/{id}/regenerate-key → 重新生成 access_key
GET    /connections/types  → 列出所有可用 connection 类型
```
...
更新或删除连接时会调用 `SchedulerService.sync_trigger()` 确保
