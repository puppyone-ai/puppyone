...
| hash | 用途 | 存储位置 |
|------|------|---------|
| `scope_hash` | CAS 比较 — 检测同 scope 并发 push | `mut_scope_state` 表，每个 scope 一行 |
| `root_hash` | 全局文件树 — 所有读操作的数据源 | `projects` 表 |
...
**理想**：版本号由数据库原子递增（`UPDATE SET v = v + 1 RETURNING v`），不可能重复。

**现状**：版本号在内存中 +1 后写回数据库（per-instance counter）。
两个并发请求可能同时读到同一个版本号 → 版本历史中出现重复。
...
def _push_attempt(repo, scope, auth, req):
    ...
    repo.record_history(new_version, ..., scope_hash=new_scope_hash)

    return PushResponse(
        status="ok", version=new_version,
        root=new_scope_hash,
...
| 组件 | 位置 |
|------|------|
| Push 处理（CAS 版） | `mut/server/handlers.py` — `handle_push` / `_push_attempt` |
...
| CAS scope 状态 | `backend/src/mut_engine/server/backends/supabase_history.py` |
| 嫁接 hook | `backend/src/mut_engine/services/hooks.py` |
| Scope 状态表 | `mut_scope_state` (Supabase) |
| 全局版本号 | `projects.mut_version` (Supabase, atomic increment) |
