...
> 最后更新：2026-02-24
> ⚠️ 本文档描述的是 Stateless Mirror 同步引擎的设计。旧端点路径 `/access/openclaw/*` 已迁移至 `/sync/openclaw/*`（生命周期）和 `/sync/{folder_id}/*`（数据同步）。
...
┌─────────────────────┐                  ┌──────────────────────────────────┐
│ chokidar 文件监听    │── push ────────▶│ /sync/{folder_id}/push           │
│                     │                  │   ↓ FolderSyncService            │
│ long-poll           │── changes ─────▶│ /sync/{folder_id}/changes        │
│                     │◀── 增量返回 ────│   ↓ sync_changelog cursor        │
└─────────────────────┘                  │   ↓ VersionService               │
                                         └──────────────────────────────────┘
```
