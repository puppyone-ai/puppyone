...
export type HtmlBridgeMessage = HtmlSelectionMessage | { type: "ready"; ids: string[] }
...
export function decodeHtmlBridgeMessage(value: unknown): HtmlBridgeMessage | null {
  ...
  for (const name of ["fontFamily", "fontSize", "fontWeight", "lineHeight", "color", "backgroundColor", "textAlign",
    "padding", "borderRadius", "width", "height", "margin", "transformed"]) {
  ...
  return { type: "selection", id: message.id, edit: message.edit, rect: { x: rect.x, y: rect.y, width: rect.width, height: rect.height },
