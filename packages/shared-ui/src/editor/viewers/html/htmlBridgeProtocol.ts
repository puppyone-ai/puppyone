...
export function decodeHtmlBridgeMessage(value: unknown): HtmlBridgeMessage | null {
  ...
  if (message.type === "viewport" && [message.x, message.y].every((n) => typeof n === "number" && Number.isFinite(n) && Math.abs(n) <= 1e7)) return { type: "viewport", x: message.x as number, y: message.y as number };
  ...
  return { type: "selection", id: message.id, edit: message.edit, anchor: { x: anchor.x, y: anchor.y, width: anchor.width, height: anchor.height }, rect: { x: rect.x, y: rect.y, width: rect.width, height: rect.height },
