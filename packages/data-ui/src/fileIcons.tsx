...
export function getFileVisualKind(name: string, type?: string | null): FileVisualKind {
  if (isFileVisualKind(type)) return type;
...
export function getFileIcon(filename: string, size = 48): ReactNode {
...
function isFileVisualKind(value: string | null | undefined): value is FileVisualKind {
  return Boolean(value && value in KIND_ACCENT);
}
...
function DefaultGlyph({
  ...
  if (kind === "markdown" || kind === "text" || kind === "file" || kind === "pdf" || kind === "json") {
    return (
...
function FolderGlyph({ size, compact = false }: { size: number; compact?: boolean }) {
