...
export function NodeProps() {
  ...
  const inactivateNode = async (nodeid: string) => {
    ...
    setNodes(prevNodes =>
      prevNodes.map(node => ({
        ...
        preventInactivated:
          node.id === nodeid ? false : node.preventInactivated,
        editable: node.id === nodeid ? false : node.editable,
        TopSrcHandle: {
          ...node.TopSrcHandle,
          activated: node.id === nodeid ? false : node.TopSrcHandle.activated,
        },
        BottomSrcHandle: {
          ...node.BottomSrcHandle,
          activated:
            node.id === nodeid ? false : node.BottomSrcHandle.activated,
        },
        LeftSrcHandle: {
          ...node.LeftSrcHandle,
          activated: node.id === nodeid ? false : node.LeftSrcHandle.activated,
        },
        RightSrcHandle: {
          ...node.RightSrcHandle,
          activated: node.id === nodeid ? false : node.RightSrcHandle.activated,
        },
      }))
    );
  ...
  const lockNode = (nodeid: string) => {
    ...
    setNodes(prevNodes =>
      prevNodes.map(node =>
        node.id === nodeid
          ? { ...node, locked: true, isInput: false, isOutput: false }
          : node
      )
    );
  };
  ...
  const unlockNode = (nodeid: string) => {
    ...
    setNodes(prevNodes =>
      prevNodes.map(node =>
        node.id === nodeid ? { ...node, locked: false } : node
      )
    );
  };
  ...
  const markNodeAsInput = (nodeid: string) => {
    ...
    setNodes(prevNodes =>
      prevNodes.map(node =>
        node.id === nodeid
          ? { ...node, isInput: true, isOutput: false, locked: false }
          : node
      )
    );
  };
  ...
  const unmarkNodeAsInput = (nodeid: string) => {
    ...
    setNodes(prevNodes =>
      prevNodes.map(node =>
        node.id === nodeid ? { ...node, isInput: false } : node
      )
    );
  };
  ...
  const markNodeAsOutput = (nodeid: string) => {
    ...
    setNodes(prevNodes =>
      prevNodes.map(node =>
        node.id === nodeid
          ? { ...node, isOutput: true, isInput: false, locked: false }
          : node
      )
    );
  };
  ...
  const unmarkNodeAsOutput = (nodeid: string) => {
    ...
    setNodes(prevNodes =>
      prevNodes.map(node =>
        node.id === nodeid ? { ...node, isOutput: false } : node
      )
    );
  };
  ...
  const allowEditLabel = (nodeid: string) => {
    setNodes(prevNodes =>
      prevNodes.map(node =>
        node.id === nodeid ? { ...node, editable: true } : node
      )
    );
  };
  ...
  const disallowEditLabel = (nodeid: string) => {
    setNodes(prevNodes =>
      prevNodes.map(node =>
        node.id === nodeid ? { ...node, editable: false } : node
      )
    );
  };
    ...
    setNodes(prevNodes => {
      return prevNodes.map(node => {
        if (node.id === nodeid) {
          ...
          switch (handlePosition) {
            case Position.Top:
              updatedNode.TopSrcHandle = {
                ...updatedNode.TopSrcHandle,
                isConnected: false,
              };
              break;
            case Position.Bottom:
              updatedNode.BottomSrcHandle = {
                ...updatedNode.BottomSrcHandle,
                isConnected: false,
              };
              break;
            case Position.Left:
              updatedNode.LeftSrcHandle = {
                ...updatedNode.LeftSrcHandle,
                isConnected: false,
              };
              break;
            case Position.Right:
              updatedNode.RightSrcHandle = {
                ...updatedNode.RightSrcHandle,
                isConnected: false,
              };
              break;
    ...
    setNodes(prevNodes =>
      prevNodes.map(node => ({
        ...
        TopSrcHandle: {
          ...node.TopSrcHandle,
          activated:
            handlePosition === Position.Top && node.id === nodeid
              ? true
              : false,
        },
        BottomSrcHandle: {
          ...node.BottomSrcHandle,
          activated:
            handlePosition === Position.Bottom && node.id === nodeid
              ? true
              : false,
        },
        LeftSrcHandle: {
          ...node.LeftSrcHandle,
          activated:
            handlePosition === Position.Left && node.id === nodeid
              ? true
              : false,
        },
        RightSrcHandle: {
          ...node.RightSrcHandle,
          activated:
            handlePosition === Position.Right && node.id === nodeid
              ? true
              : false,
        },
      }))
      ...
      prevNodes.map(node => ({
        ...
        BottomSrcHandle: {
          ...
          activated:
            handlePosition === Position.Bottom && node.id === nodeid
              ? false
              : node.BottomSrcHandle.activated,
        },
        LeftSrcHandle: {
          ...node.LeftSrcHandle,
          activated:
  ...
  const activateEdgeNode = (edgeNodeid: string) => {
    ...
    setEdgeActivate(prevEdgeNodeId =>
      prevEdgeNodeId === edgeNodeid ? '-1' : edgeNodeid
    );
  };
  ...
  const restore = (
    ...
    let lastnodes: nodeState[] = prevNodes
      .filter(
        node =>
          !isNaN(parseInt(node.id)) && String(parseInt(node.id)) === node.id
      )
      .map(node => ({
        id: node.id,
        activated: false,
...
export const NodeContextProvider = ({
  children,
}: providerType): ReactElement => {
  return (
    <NodeContext.Provider value={NodeProps()}>{children}</NodeContext.Provider>
  );
};
