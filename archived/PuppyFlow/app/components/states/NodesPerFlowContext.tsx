...
export const NodesPerFlowContext = createContext<nodesPerFlowContextType>(
  initialNodesPerFlowContext
);
...
export function NodesPerFlowUtils() {
  ...
  const setNodeEditable = useCallback((nodeId: string) => {
    setNodes(nodes =>
      nodes.map(node =>
        node.id === nodeId
          ? { ...node, data: { ...node.data, editable: true } }
          : node
      )
    );
  }, []);
  ...
  const setNodeUneditable = useCallback((nodeId: string) => {
    setNodes(nodes =>
      nodes.map(node =>
        node.id === nodeId
          ? { ...node, data: { ...node.data, editable: false } }
          : node
      )
    );
  }, []);
  ...
  const setHandleActivated = useCallback(
    (nodeId: string, handlePosition: Position | null) => {
...
export const NodesPerFlowContextProvider = ({
  children,
