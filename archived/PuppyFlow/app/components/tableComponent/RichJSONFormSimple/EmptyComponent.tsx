...
const EmptyComponent = ({
    ...
    <div className='flex items-center space-x-3 p-2'>
      ...
      <div className='flex-1'>
        <div className='text-white font-medium text-sm flex items-center'>
          {option.label}
          {option.type === 'text' && (
            <span className='ml-2 text-xs text-[#888] bg-[#333] px-1 py-0.5 rounded'>
              Default
            </span>
          )}
        </div>
        <div className='text-[#888] text-xs'>{option.description}</div>
      </div>
    </div>
  );
  ...
  return (
    <div
      className={`w-full bg-transparent rounded-md border-2 border-dashed border-[#555] 
