...
const TreeNode: React.FC<TreeNodeProps> = ({
  ...
  const hasChildren =
    (isObject && Object.keys(data as JSONObject).length > 0) ||
  ...
  const renderKey = () => {
    ...
    return (
      <span
        className={`${getKeyColor()} ${getKeyBackgroundStyle()} font-medium mr-2 select-none px-1.5 py-0.5 rounded text-xs`}
      >
        {key}:
      </span>
    );
  ...
  const renderValue = () => {
    if (isObject) {
      if (hasChildren) {
        return (
          ...
          >
            ...
            <span>
              {isExpanded
                ? ''
                : ` ${Object.keys(data as JSONObject).length} items `}
            </span>
        ...
        return (
          <span className='text-[#B794F6] select-none font-medium'>{'{'}</span>
        );
      }
    }
      ...
      if (hasChildren) {
        return (
          ...
          >
            ...
            <span>
              {isExpanded ? '' : ` ${(data as JSONArray).length} items `}
            </span>
            <span>{isExpanded ? '' : ']'}</span>
          </button>
        );
        ...
        return (
          <span className='text-[#F6AD55] select-none font-medium'>{'['}</span>
        );
      }
    }
      ...
      <div className='flex items-center gap-1 ml-2'>
        {(isObject || isArray) && (
          <div className='relative'>
            ...
            >
              <svg
                className='w-3 h-3 text-[#6D7177] group-hover:text-[#9CA3AF]'
                fill='currentColor'
                viewBox='0 0 20 20'
              >
                <path
        ...
        {!isRoot && (
          ...
          >
            <svg
              className='w-3 h-3 text-[#6D7177] group-hover:text-[#FF6B6B]'
              fill='currentColor'
              viewBox='0 0 20 20'
            >
              <path
