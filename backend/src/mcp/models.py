...
class McpInstance(BaseModel):
    ...
    status: int = Field(default=..., description="MCP server状态，0表示关闭，1表示开启")
    port: Optional[int] = Field(
        default=None, deprecated=True, description="端口号，已废弃"
    )
    docker_info: Optional[Dict[Any, Any]] = Field(
        default=None, deprecated=True, description="容器信息或进程信息，已废弃"
    )
    tools_definition: Optional[Dict[ToolTypeKey, McpToolsDefinition]] = Field(
        default=None, description="(可选) 工具定义的字典"
    )
    register_tools: Optional[List[ToolTypeKey]] = Field(
        default=None, description="(可选) 已注册的工具列表，为空表示都注册"
    )
