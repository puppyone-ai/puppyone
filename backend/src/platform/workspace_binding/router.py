...
from src.platform.workspace_binding.schemas import (
    CanonicalProjectSummaryOut,
    CanonicalRemoteContextOut,
    CanonicalRemoteResolveRequest,
    GitRemoteOut,
...
def create_workspace_binding(
    ...
    authorized: AuthorizedProject = Depends(require_project_action(ProjectAction.BIND_READONLY)),
    ...
    return ApiResponse.success(
        data=_out(
            ...
            usable=usable,
            reason=reason,
            capabilities=sorted(capability.value for capability in authorized.grant.capabilities),
        ),
...
def list_workspace_bindings(
    ...
    bindings = service.list_for_project(project_id, current_user.user_id, all_users=all_users)
...
def get_workspace_binding(
    ...
    binding, usable, reason, grant = service.get(binding_id, current_user.user_id)
    return ApiResponse.success(
        data=_out(
            binding,
            usable=usable,
...
def heartbeat_workspace_binding(
    ...
    binding, usable, reason, grant = service.heartbeat(binding_id, current_user.user_id)
    return ApiResponse.success(
        data=_out(
            binding,
            usable=usable,
...
def rotate_workspace_binding_credential(
    ...
    return ApiResponse.success(
        data=WorkspaceBindingCredentialOut(
            ...
            credential=credential,
            remote=_out(service.get(binding_id, current_user.user_id)[0]).remote,
        )
    )
...
def resolve_legacy_remote(
    ...
    target = service.resolve_legacy_remote(
        ...
        expected_origin=(settings.PUBLIC_URL or f"{request.url.scheme}://{request.url.netloc}"),
    )
    return ApiResponse.success(
...
def resolve_canonical_remote(
    payload: CanonicalRemoteResolveRequest,
    request: Request,
    current_user: CurrentUser = Depends(get_current_user),
    service: WorkspaceBindingService = Depends(get_workspace_binding_service),
):
