...
def process_version_outbox_batch(
    *,
    ...
    if not settings.VERSION_OUTBOX_ENABLED:
        return 0

    db = client or SupabaseClient().client
    repos = repo_manager or get_repo_manager_standalone()
    rows = _claim_rows(db, limit or settings.VERSION_OUTBOX_BATCH_SIZE)
    processed = 0
...
def complete_version_outbox_for_commit(
    ...
    *,
    ...
    The scope-publish RPC always inserts an outbox row in the same DB
    ...
    if not settings.VERSION_OUTBOX_ENABLED or not project_id or not commit_id:
        return 0

    db = client or SupabaseClient().client
    try:
        resp = (
            db.table(VERSION_OUTBOX_TABLE)
            .update({
...
def _complete_row(client, row_id) -> None:
    if row_id is None:
        return
    client.rpc(COMPLETE_OUTBOX_RPC, {"p_id": row_id}).execute()
