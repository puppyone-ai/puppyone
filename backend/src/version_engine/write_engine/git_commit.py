"""Git commit object helpers used by the Write Engine."""
...
def shallow_git_parent_or_empty(repo, commit_id: str) -> str:
    """Return ``commit_id`` when it is safe to use as a Git parent cheaply.

    This intentionally validates only the candidate commit object and its tree,
    not the full ancestry graph. Request-path product writes already publish
    through PuppyOne's DB ledger; walking every parent on each save turns a
    one-file edit into an O(history) S3 scan. Git transport/view code can still
    run the deeper compatibility projection at clone/fetch boundaries.
    """
