...
def build_tree_delta(
    ...
    *,
    ...
    _diff_tree_recursive(store, old_tree, new_tree, normalize_path(prefix), changes, tolerant)
...
def _diff_tree_recursive(
    ...
    if old_tree == new_tree:
        return
    left = _read_tree_or_empty(store, old_tree, tolerant)
    right = _read_tree_or_empty(store, new_tree, tolerant)
    for name in sorted(set(left) | set(right)):
        ...
        if old_entry is None:
            ...
            if old_type == "T" and new_type == "T":
                _diff_tree_recursive(store, old_oid, new_oid, path, out, tolerant)
            else:
                out.append(
                    TreeChange(
                        path=path,
...
def _read_tree_or_empty(store: ObjectStore, tree_hash: str, tolerant: bool) -> dict:
    if not tree_hash:
        return {}
    try:
        return tree_mod.read_tree(store, tree_hash)
    except Exception as exc:  # noqa: BLE001 - tolerant read-path degrades gracefully
        if tolerant:
            log_warning(f"[tree_delta] tree {tree_hash[:12]} unreadable, treating as empty: {exc}")
            return {}
        raise
