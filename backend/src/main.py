...
sandbox_router_start = time.time()
from src.sandbox.router import router as sandbox_router

sandbox_router_duration = time.time() - sandbox_router_start
...
async def app_lifespan(app: FastAPI):
    ...
    log_info(f"  │  ├─ sandbox_router: {sandbox_router_duration * 1000:.2f}ms")
    log_info(
...
def create_app() -> FastAPI:
    ...
    app.include_router(sandbox_router, prefix="/api/v1", tags=["sandboxes"])
