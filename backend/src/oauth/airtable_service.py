...
class AirtableOAuthService:
    ...
    async def handle_callback(self, user_id: str, code: str, state: Optional[str] = None) -> tuple[bool, str, Optional[dict]]:
        ...
        try:
            ...
            workspace_name = user_info.get("email") or user_info.get("name") or "Airtable"
            ...
            return True, "Successfully connected to Airtable", {
                "workspace": workspace_name,
                "connection_id": connection.id,
            }
        except httpx.HTTPStatusError as err:
            error_message = f"Airtable token exchange failed: {err.response.status_code}"
            try:
