...
class ETLConfig(BaseSettings):
    ...
    etl_redis_prefix: str = Field(
        default="etl:",
        description="Redis key prefix for ETL runtime state"
    )

    etl_state_ttl_seconds: int = Field(
        default=24 * 60 * 60,
        description="TTL for ETL runtime state in Redis (seconds)"
    )

    etl_state_terminal_ttl_seconds: int = Field(
        default=60 * 60,
        description="TTL for terminal state cache in Redis (seconds)"
    )

    etl_arq_queue_name: str = Field(
        default="etl",
        description="ARQ queue name for ETL jobs"
    )

    etl_ocr_max_attempts: int = Field(
        default=3,
        description="Maximum attempts for OCR stage"
    )

    etl_postprocess_max_attempts: int = Field(
        default=3,
        description="Maximum attempts for postprocess stage"
    )

    etl_retry_backoff_base_seconds: int = Field(
        default=2,
        description="Base seconds for retry backoff"
    )
    ...
    etl_postprocess_chunk_size_chars: int = Field(
        default=12_000,
        description="Chunk size for chunked strategies"
    )

    etl_postprocess_max_chunks: int = Field(
        default=20,
        description="Maximum number of chunks for chunked strategies"
    )

    etl_global_rule_enabled: bool = Field(
        default=True,
        description="Enable built-in global default ETL rule"
    )
