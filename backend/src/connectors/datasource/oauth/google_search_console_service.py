...
class GoogleSearchConsoleOAuthService:
    """Service for handling Google Search Console OAuth flow."""
    ...
    async def get_authorization_url(
        ...
        return f"{self.GOOGLE_OAUTH_URL}?{query_string}", state
    ...
    async def handle_callback(
        ...
        try:
            ...
            connection = await self.repository.create(OAuthConnectionCreate(
                user_id=user_id,
            ...
            message = f"Google token exchange failed: {err.response.status_code}"
            try:
                message = err.response.json().get("error_description", message)
            except Exception:
                pass
            return False, message, None
        except Exception as exc:
            return False, f"Failed to handle Google Search Console callback: {exc!s}", None

    async def get_connection(self, user_id: str) -> Optional[OAuthConnection]:
        return await self.repository.get_by_user_and_provider(
            user_id, "google-search-console",
        )
