...
from mut.foundation.git_format import encode_object
...
async def stage_blob_from_s3(
    ...
    *,
    ...
    """Stage an uploaded file as a Git-compatible MUT blob.

    The source object is the user's raw file bytes. The MUT object
    store now expects Git loose-object bytes, so the worker must
    write ``zlib(b"blob <size>\\0" + content)`` under the Git blob
    SHA-1. A direct S3 ``CopyObject`` would store the wrong bytes.
    """
...
async def _stage_blob_for_mut(
    ...
    *,
    ...
    Encodes ``content`` as a Git blob object and writes the loose
    object bytes to the MUT key. New callers should use
    :func:`stage_blob_from_s3`.
    """
...
async def finalize_upload_to_mut(
    *,
    ...
    try:
        ...
        # Stage the raw upload as a Git-compatible blob object and
        # return the resulting Git object id for the tree update.
...
async def finalize_uploads_to_mut_batch(
    *,
    ...
    # Phase 2: stage each raw upload from S3 as a Git blob object
    # under its MUT object key.
    #
    # Old flow (deleted):
    ...
    #
    # ``ops.bulk_write_refs`` only sees ``(hash, size)`` for the
    # commit, and the staged object is byte-compatible with Git.
    #
    ...
    #
    # ``verify_blobs=False`` is safe here because we just wrote each
    # blob to its MUT key inside ``stage_blob_from_s3`` above — they
    # ARE present, no need to round-trip a HEAD per ref.
    from src.mut_engine.dependencies import create_mut_ops
    ops = create_mut_ops()
