    ...
    def merge_data_at_path(
        ...
        try:
            ...
            try:
                ...
                if not isinstance(current, list):
                    raise BusinessException(
                        f"Path error: target is not an array",
                        code=ErrorCode.BAD_REQUEST
                    )
                ...
                if not isinstance(current, dict):
                    raise BusinessException(
                        f"Path error: target is not an object",
                        code=ErrorCode.BAD_REQUEST
                    )
                # 合并到对象字段
    ...
    def _merge_values(
        ...
        if strategy == "replace":
            ...
            if isinstance(current_value, dict) and isinstance(import_value, dict):
                ...
                # 如果类型不匹配，直接替换
                log_warning(f"merge_object: type mismatch, replacing instead")
                return import_value
            ...
            if isinstance(current_value, list):
                ...
                # 如果当前不是数组，直接替换
                log_warning(f"append_array: current value is not array, replacing instead")
                return import_value
            ...
            if mode == "replace_all":
                ...
                if original_data is None:
                    ...
                    if isinstance(import_data, list):
                        # 导入数据也是数组，合并
                        log_info(f"Import is also array ({len(import_data)} items), extending")
                    ...
                    if isinstance(import_data, dict):
                        ...
                        log_info(f"Import is object: {len(new_fields)} new fields, {len(updated_fields)} updated")
                    ...
                    log_info(f"Current is primitive type: {type(original_data).__name__}")
                ...
                if not isinstance(original_data, dict):
                    log_info(f"Current is not object ({type(original_data).__name__}), restructuring")
