"""WebSocket router for the MUT real-time notification stream.

Mirrors the ``/ws`` endpoint that ``mut listen`` (client-side, see
...
* Each ``commit_update`` event is JSON, shaped to match
  ``mut.server.notification.Notification.to_dict()`` so the client's
  ``store_message`` lands a payload it knows how to render.
...
_BEARER_SUBPROTOCOL_PREFIX = "mut.bearer."
_ACCEPT_SUBPROTOCOL = "mut.v1"
...
def _extract_ws_credentials(websocket: WebSocket) -> tuple[str, str | None]:
    ...
    2. ``Sec-WebSocket-Protocol: mut.bearer.<token>`` — the only viable
       path from a browser, since ``new WebSocket(url, ...)`` cannot
       set arbitrary headers. We must echo back *some* subprotocol so
       the handshake completes; we choose a benign ``mut.v1`` rather
...
async def mut_ws(websocket: WebSocket, project_id: str):
    ...
    ``X-Mut-User`` header / ``user`` query param scopes the audit
    ...
    user_identity = (
        websocket.headers.get("x-mut-user")
    ...
    try:
        ...
        log_error(f"[MUT-WS] auth failed for project={project_id}: {e}")
        await websocket.close(code=status.WS_1008_POLICY_VIOLATION,
                              reason="auth failed")
        return
