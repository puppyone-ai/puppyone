...
``agent_review`` pending transactions:
  ...
  GET  /api/v1/content/{project_id}/conflicts/pending
       List ``mut_conflicts`` rows with ``status='pending'`` for this
...
from src.mut_engine.routers._content_helpers import (
...
async def list_pending_conflicts(
    ...
    """Return every ``mut_conflicts`` row with ``status='pending'``.
