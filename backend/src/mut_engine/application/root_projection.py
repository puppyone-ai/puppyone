...
def rebuild_project_root_after_commit(repo, push_result: dict) -> bool:
    ...
    function reads ``mut_scope_state`` (the source of truth), grafts the
    ...
    for attempt in range(_MAX_GRAFT_RETRIES):
        try:
            ...
            if hasattr(repo, "cas_update_root_hash"):
                ...
                # Fallback for in-memory test repos that don't support CAS.
...
def record_project_view_index_for_commit(
    *,
    ...
    Writes a row to ``mut_version_index`` mapping the canonical scope
