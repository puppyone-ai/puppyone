"""tree_splice — direct Merkle-tree mutations for typed write operations.
...
the same order MUT's history schema expects. The caller is responsible for
prefixing ``rel_path`` with the scope path before persisting.

Cost: ``O(D)`` per mutation where ``D`` is the depth of the affected path
(typically < 10), independent of total scope size. Compare with MUT's
...
def _rebuild_spine(
    ...
    if cur_hash is None:
        ...
        # have a real hash to write to ``mut_scope_state``.
...
def splice_put_blob_ref(
    ...
    **Caller contract**: ``blob_hash`` MUST already be present in
    ...
    server-side ``CopyObject`` into the MUT object key). Skipping
