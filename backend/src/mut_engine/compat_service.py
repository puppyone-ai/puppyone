...
class MutCompatService:
    ...
    async def _do_node_create(self, m: Mutation) -> CommitResult:
        ...
        node = await self._create_node_in_db(m)
    ...
    async def rollback_file(
        ...
        new_version = await self._mut.rollback(node.project_id, target_version, operator)
    ...
    async def _create_node_in_db(self, m: Mutation):
        content_bytes = _serialize_content(m.content, m.node_type)
        ...
        if m.node_type == "json":
            ...
            return await self._node_svc.create_markdown_node(
                project_id=m.project_id,
                name=m.name,
                parent_id=m.parent_id,
                created_by=m.created_by,
...
CollaborationService = MutCompatService
