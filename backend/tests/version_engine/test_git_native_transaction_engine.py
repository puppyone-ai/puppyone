...
from src.version_engine.application.git_object_format import encode_commit
...
from src.version_engine.application.tree_objects import build_tree_from_files, flatten_tree_to_bytes
...
async def test_git_fast_forward_push_uses_tree_diff_not_full_flatten(
    ...
    monkeypatch.setattr(
        "src.version_engine.application.transaction_engine.flatten_tree_to_bytes",
        fail_flatten,
    )
...
def _patch_git_access_points(monkeypatch, mapping: dict[str, tuple]):
    ...
    monkeypatch.setattr(
        "src.version_engine.adapters.git.router.resolve_access_point",
        fake_resolve,
    )
...
class TestGitNativeHardeningContracts:
    ...
    async def test_project_operation_publish_does_not_wait_for_sync_projection(
        ...
        from src.version_engine.services import hooks
        ...
        result = await GitNativeTransactionEngine(repo_manager).apply_project_operation(
            OperationWriteIntent(
                project_id="test-proj",
                scope_path="",
                actor="user:save",
...
def test_version_outbox_worker_replays_hook_and_marks_complete(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.version_engine.services.version_outbox.run_post_push_hook",
        fake_hook,
    )
...
def test_version_outbox_worker_routes_project_rows_to_project_hook(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.version_engine.services.version_outbox.run_post_project_update_hook",
        fake_project_hook,
    )
    monkeypatch.setattr(
        "src.version_engine.services.version_outbox.run_post_push_hook",
