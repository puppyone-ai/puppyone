...
class TestCasBackoff:
    def test_first_attempt_does_not_sleep(self, monkeypatch):
        from src.version_engine.write_engine import engine
        ...
        asyncio.run(engine._cas_backoff(0))
    ...
    def test_later_attempts_sleep_within_bounds(self, monkeypatch):
        ...
        for attempt in range(1, 6):
            asyncio.run(engine._cas_backoff(attempt))

        assert len(slept) == 5
        ceiling_s = engine._CAS_BACKOFF_MAX_MS / 1000.0
        for delay in slept:
