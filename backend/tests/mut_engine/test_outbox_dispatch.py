...
from src.mut_engine.services import version_outbox
...
def _fake_client(rows: list[dict[str, Any]]) -> MagicMock:
    ...
    client = MagicMock()
    # claim_mut_version_outbox_batch returns the rows once, then nothing.
    seen = {"called": False}

    def _rpc(fn_name: str, _args=None):
        if fn_name == "claim_mut_version_outbox_batch":
            execute = MagicMock()
...
def _enable_outbox(monkeypatch):
    ...
    import src.mut_engine.services.version_outbox as mod
    monkeypatch.setattr(mod.settings, "MUT_VERSION_OUTBOX_ENABLED", True)
...
def test_unknown_event_type_is_completed_not_failed(monkeypatch):
    ...
    # call fail_mut_version_outbox; we count which RPC happened.
    rpc_calls: list[str] = []
    ...
    def _rpc(fn_name: str, _args=None):
        ...
        if fn_name == "claim_mut_version_outbox_batch":
            ...
            ] if not rpc_calls.count("claim_mut_version_outbox_batch") > 1 else []))
        else:
            execute.execute.return_value = MagicMock(data=[])
        return execute
    ...
    assert processed == 1
    assert "complete_mut_version_outbox" in rpc_calls
    assert "fail_mut_version_outbox" not in rpc_calls
