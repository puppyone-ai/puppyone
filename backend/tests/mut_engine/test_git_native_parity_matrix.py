...
This file mirrors the broad MUT test intent with fast Git-native contract
matrices. The goal is not artificial line count: each matrix fixes one
semantic axis that Git clients must inherit from PuppyOne/MUT collaboration.
"""
...
def test_manual_review_policy_merge_matrix(
    ...
    _ = case_name
    result = merge_file_sets_for_manual_review(base, current, incoming)
