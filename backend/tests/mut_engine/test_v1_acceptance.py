...
K7  Pending → resolve(accept) produces a final accepted commit and a
    clean audit chain (mut_conflicts goes pending → resolved with a
...
K8  Project Git history shows child-scope commits via projected
    commits (``mut_version_index`` mapping is recorded).
...
async def test_k7_pending_resolve_produces_clean_audit_chain(
    ...
    import src.mut_engine.application.transaction_engine as engine_mod
...
async def test_k8_child_scope_commit_appears_in_project_view_mapping(
    ...
    ``mut_version_index`` row mapping the child commit to a project-view
    ...
    assert matched, (
        f"expected a mut_version_index row for child scope commit "
...
async def test_k9_read_your_write_after_apply_operation(
    ...
    from src.mut_engine.application.git_commit import commit_tree_id
