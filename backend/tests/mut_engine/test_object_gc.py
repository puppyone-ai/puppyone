...
from src.mut_engine.services.object_gc import (
...
def test_git_object_gc_keeps_legacy_raw_tree_children_reachable(server_repo):
    legacy_blob = _put_raw_object(server_repo.store, b"legacy bytes")
    legacy_tree = _put_raw_object(
        server_repo.store,
        json.dumps({"legacy.txt": ["B", legacy_blob]}).encode("utf-8"),
    )
    orphan_raw = _put_raw_object(server_repo.store, b"raw orphan")

    repo = server_repo
    repo.history.set_scope_hash("", legacy_tree)
    repo.history.set_root_hash(legacy_tree)

    roots = collect_object_gc_roots(repo)
    reachable = mark_reachable_objects(repo, roots)
    assert legacy_tree in reachable
    assert legacy_blob in reachable

    result = run_git_object_gc(repo, dry_run=False, retention_seconds=0)

    assert result.deleted_count == 1
    assert repo.store.exists(legacy_tree)
    assert repo.store.exists(legacy_blob)
...
def test_object_gc_worker_can_run_bounded_manual_project_list(server_repo, monkeypatch):
    ...
    monkeypatch.setattr(
        "src.mut_engine.services.object_gc_worker.settings.MUT_OBJECT_GC_ENABLED",
        False,
    )
