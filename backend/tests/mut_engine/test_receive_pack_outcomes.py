...
What we verify:
  ...
  * legacy ``ok=`` kwarg still routes to committed/rejected.
...
from src.mut_engine.adapters.git.receive_pack import receive_pack_result
...
def test_legacy_ok_true_is_committed():
    resp = receive_pack_result(REF, ok=True, message="ok")
    assert f"ok {REF}".encode() in resp.body


def test_legacy_ok_false_is_rejected():
    resp = receive_pack_result(REF, ok=False, message="boom")
    assert f"ng {REF} boom".encode() in resp.body


def test_missing_both_outcome_and_ok_raises():
    with pytest.raises(ValueError, match="outcome= or ok="):
