# etl-core Specification Deltas (简化版 MVP)
...
系统 SHALL 支持用户自定义 ETL 规则（JSON Schema + system_prompt）。
...
- **AND** 生成唯一 rule_id
- **AND** 保存规则到 `.etl_rules/{rule_id}.json` 文件
- **AND** 返回 rule_id
...
- **WHEN** 用户查询规则
- **AND** 提供 rule_id
- **THEN** 应从 `.etl_rules/{rule_id}.json` 读取规则
- **AND** 返回完整规则对象
...
- **WHEN** 用户请求删除规则
- **AND** 提供 rule_id
- **THEN** 应删除 `.etl_rules/{rule_id}.json` 文件
- **AND** 返回 204 状态码
- **AND** 如果规则不存在，返回 404 错误
...
- **AND** 任务状态设置为 "pending"
- **AND** 将任务 ID 添加到队列
- **AND** 立即返回 202 状态码和 task_id

#### Scenario: Worker 消费任务

- **WHEN** Worker 从队列获取任务 ID
- **THEN** 应更新任务状态为 "mineru_parsing"
- **AND** 执行完整 ETL 流程
- **AND** 更新任务进度
- **AND** 完成后更新状态为 "completed" 或 "failed"

#### Scenario: 任务状态持久化

- **WHEN** 任务状态更新
- **THEN** 应将任务信息持久化到内存字典
- **AND** 包含: task_id、status、progress、created_at、updated_at、result、error
...
系统 SHALL 实现完整的 ETL 服务流程，从原始文件到结构化 JSON。
...
- **AND** 路径为 `/users/{user_id}/raw/{project_id}/{filename}`
...
- **AND** 更新任务进度为 "llm_processing"

#### Scenario: 步骤6-7 - 应用规则

- **WHEN** 应用 ETL 规则
- **THEN** 应从 repository 加载规则
- **AND** 调用规则引擎进行转换
- **AND** 验证输出符合 Schema
...
- **AND** 路径为 `/users/{user_id}/processed/{project_id}/{filename}.json`
...
系统 SHALL 提供 API 接口查询 ETL 任务的状态和结果。
...
- **THEN** 应使用相应的 HTTP 状态码:
  - 400: 无效参数（InvalidRule、InvalidSchema）
  - 404: 资源不存在（TaskNotFound、RuleNotFound、FileNotFound）
