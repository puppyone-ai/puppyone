...
所有 Access Point 存储在同一张 `access_points` 表中，通过 `provider`（提供方）和 `direction`（方向）区分类型。这意味着你可以用一套统一的接口来管理所有外部集成，不需要记忆不同的操作方式。
...
PuppyOne CLI 提供统一的 `access` 命令来管理所有类型的连接：

```bash
# 添加连接
puppyone access add notion <url>         # 添加 Notion 数据源
puppyone access add github <repo>        # 添加 GitHub 仓库
puppyone access add mcp "My Endpoint"    # 创建 MCP 端点
puppyone access add agent "My Agent"     # 创建 Agent
puppyone access add sandbox "My Box"     # 创建沙盒
puppyone access add filesystem ~/my-docs     # 挂载本地文件夹

# 查看连接
puppyone access ls                       # 列出所有连接
puppyone access info <id>                # 查看连接详情

# 管理连接
puppyone access pause <id>               # 暂停连接
puppyone access resume <id>              # 恢复连接
puppyone access rm <id>                  # 删除连接
```

### 示例：创建 MCP 端点并查看详情

```bash
$ puppyone access add mcp "产品知识库 MCP"
