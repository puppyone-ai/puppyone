...
Access Point 是 PuppyOne 统一的外部集成抽象。所有类型的外部集成都通过 `access_points` 表管理，使用 `provider` 和 `direction` 字段区分类型。
...
- 通过 `access_points.config.scope` 配置 Agent 可访问的路径范围
- 支持只读、读写等权限级别
- 遵循最小权限原则
