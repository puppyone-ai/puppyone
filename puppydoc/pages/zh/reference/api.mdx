...
```
GET /nodes/by-mut-path/?project_id={project_id}&mut_path=folder/file.json
```
...
```
GET /access/{connection_id}
```

### 更新连接

```
PATCH /access/{connection_id}
```

### 删除连接

```
DELETE /access/{connection_id}
```

### 重新生成访问密钥

```
POST /access/{connection_id}/regenerate-key
```
...
DELETE /db-connector/access/{connection_id}                    # 删除 Access
