...
Content API 是 PuppyOne 的核心数据接口，通过**路径**寻址（而非 UUID）。所有内容操作统一使用 `/content/{project_id}/...` 前缀。

### 浏览与读取

```
GET /content/{project_id}/ls?path=                             # 列出目录
GET /content/{project_id}/cat?path=                            # 读取文件内容
GET /content/{project_id}/stat?path=                           # 获取文件/目录元信息
GET /content/{project_id}/tree?path=&max_depth=                # 完整目录树
GET /content/{project_id}/trash                                # 列出回收站
```

### 创建与写入

```
POST /content/{project_id}/mkdir                               # 创建目录
POST /content/{project_id}/write                               # 写入文件
POST /content/{project_id}/bulk-write                          # 批量写入
```
...
```
POST /content/{project_id}/mv                                  # 移动或重命名
POST /content/{project_id}/rm                                  # 删除 / 移入回收站
POST /content/{project_id}/restore                             # 从回收站恢复
```
...
```
GET  /content/{project_id}/versions?path=&limit=               # 版本历史
GET  /content/{project_id}/version-content?path=&version=      # 指定版本内容
GET  /content/{project_id}/diff?v1=&v2=                        # 对比两个版本
POST /content/{project_id}/rollback                            # 回滚到指定版本
```
...
PuppyOne 提供完整的协作编辑工作流，包括 Checkout/Commit 和版本管理。版本管理通过 Content API（见第 4 节）统一处理。
...
版本历史、回滚、对比等操作请使用 Content API：

```
GET  /content/{project_id}/versions?path=                      # 版本历史
POST /content/{project_id}/rollback                            # 回滚到指定版本
GET  /content/{project_id}/diff?v1=&v2=                        # 版本对比
```
...
```
POST   /db-connector/access                                    # 创建数据库 Access
GET    /db-connector/access                                    # 列出 Access
DELETE /db-connector/access/{id}                               # 删除 Access
GET    /db-connector/access/{id}/tables                        # 列出数据库表
GET    /db-connector/access/{id}/tables/{table}/preview        # 预览表数据
POST   /db-connector/access/{id}/save                          # 保存为内容节点
```
