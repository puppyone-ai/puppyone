...
```
  ...
  ─ Sync with MUT:
    mut clone http://api.puppyone.ai/mut/ap/cli_xxx --credential cli_xxx --dir <folder>

  ─ Then commit and push:
    cd <folder> && mut commit -m "message" && mut push
...
Copy the `mut clone` command from the output and replace `<folder>` with your desired local directory:

```bash
mut clone http://api.puppyone.ai/mut/ap/cli_xxx --credential cli_xxx --dir ~/my-docs
```

This creates a MUT repository in `~/my-docs` and pulls any existing cloud content.
...
# Clone to different local directories
mut clone <docs-url> --credential <docs-key> --dir ~/my-docs
mut clone <code-url> --credential <code-key> --dir ~/my-code
```
...
```
Root/
├── docs/          ← contents of ~/my-docs
│   ├── api.md
│   └── guide.md
└── code/          ← contents of ~/my-code
    ├── main.py
    └── utils.py
```
...
| Command | Description |
|------|------|
| `mut clone <url> --credential <key> --dir <folder>` | Clone remote repo to local |
