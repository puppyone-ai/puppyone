...
All access points are stored in the same `access_points` table and distinguished by fields like `provider` and `direction`. That gives you one consistent interface for every external integration instead of separate models and workflows.
...
PuppyOne CLI provides a single `access` command for every connection type:

```bash
# Add connections
puppyone access add notion <url>         # Add a Notion data source
puppyone access add github <repo>        # Add a GitHub repository
puppyone access add mcp "My Endpoint"    # Create an MCP endpoint
puppyone access add agent "My Agent"     # Create an agent
puppyone access add sandbox "My Box"     # Create a sandbox
puppyone access add filesystem ~/my-docs     # Mount a local folder

# View connections
puppyone access ls                       # List all connections
puppyone access info <id>                # View connection details

# Manage connections
puppyone access pause <id>               # Pause a connection
puppyone access resume <id>              # Resume a connection
puppyone access rm <id>                  # Delete a connection
```

### Example: Create an MCP endpoint and inspect it

```bash
$ puppyone access add mcp "Product Knowledge Base MCP"
