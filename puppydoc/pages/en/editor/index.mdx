...
Each Content Node has its own path, access control, and version history. Content is stored in a Mut Merkle tree backed by S3, with every write creating a versioned commit.
...
Every change is automatically saved as a Mut commit:
