...
const TextConfigEditor = ({preventParentDrag, 
                    allowParentDrag,
                    parentId, 
                    placeholder = "",
                    widthStyle = 0,
                    heightStyle=0}:TextConfigEditorProps) => {
    ...
    const textConfigEditorRef = useRef<HTMLDivElement>(null)
    ...
    const handleChange: OnChange = (newValue: string | undefined) => {
      ...
      const handleEditorDidMount: OnMount = (editor, monaco) => {
        ...
        //   console.log("Editor lost focus");
         ...
         if (textConfigEditorRef.current) {
          textConfigEditorRef.current.classList.add('text-config-editor');
        }
  ...
  return (
    <div ref={textConfigEditorRef} className={`relative flex  justify-center items-center border-[1px] p-[16px] pl-[22px] border-[#6D7177] border-solid rounded-[4px] cursor-pointer  bg-[#000000] `}
    style={{width: widthStyle, height: heightStyle}}>
    {isEmpty && (
      <div className="absolute w-full h-full flex items-start justify-start p-[16px] pl-[22px] text-[#6D7177] text-[12px] font-[700] leading-normal pointer-events-none z-[10] font-plus-jakarta-sans">
        {placeholder}
      </div>
    )}
    <Editor
      className='text-config-editor'
      defaultLanguage="text"
      // theme={themeManager.getCurrentTheme()}
      width={widthStyle - 38}
      height={heightStyle - 32}
