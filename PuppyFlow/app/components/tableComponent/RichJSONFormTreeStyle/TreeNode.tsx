...
const TreeNode: React.FC<TreeNodeProps> = ({
    ...
    const getValueTypeAndPreview = () => {
        if (data === null) return { type: 'null', preview: 'null', color: '#569cd6' };
        if (data === undefined) return { type: 'undefined', preview: 'undefined', color: '#569cd6' };
    ...
    const deleteChild = (childKey: string | number) => {
        if (isArray) {
            const newArray = data.filter((_: any, index: number) => index !== childKey);
            onUpdate(newArray);
        } else if (isObject) {
