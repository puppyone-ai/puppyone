...
function AddNodeButton() {
  ...
  return (
    <div id="nodeMenuButtonContainer">
      <button id="nodeMenuButton" className={`w-[44px] h-[44px] rounded-[16px] flex flex-row items-center justify-center cursor-pointer ${selectedMenu === 1 ? "bg-[#CDCDCD]" : "bg-main-blue"} transition-colors ${isOnGeneratingNewNode ? "pointer-events-none" : "pointer-events-auto"}`}>
...
function NodeMenu({selectedMenu, clearMenu}: {selectedMenu: number, clearMenu: () => void}) {
  ...
  useEffect(() => {
    ...
    if (mousePosition) {
      ...
      const getNodeSize = (nodeType: string) => {
        switch (nodeType) {
          case "serverText":
            return { width: 160, height: 120 }  // server node 的特殊尺寸
          case "text":
          case "structured":
          case "file":
          case "weblink":
