...
export const useWorkspaceInitialization = () => {
  ...
  const initializeAllData =
    useCallback(async (): Promise<InitializationResult | null> => {
      ...
      try {
        ...
        if (userData.workspaces.length > 0) {
          ...
          try {
            const content = await workspaceManagement.fetchWorkspaceContent(
            ...
            console.error(
              `Failed to fetch content for default workspace:`,
              error
            );
            defaultWorkspace = {
              workspace_id: firstWorkspace.workspace_id,
