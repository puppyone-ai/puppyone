...
function getIntersectionPointCoordsByPosition(node:InternalNode, handlePosition:Position) {
...
function getNodeCenter(node:InternalNode) {
    if (!node.measured || !node.measured.width || !node.measured.height) return {x:0,y:0}
