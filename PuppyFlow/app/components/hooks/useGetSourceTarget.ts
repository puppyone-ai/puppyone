...
function useGetSourceTarget() {
    ...
    const nodeTypeCategories = {
        ...
        edgenode: [
            'copy', 'chunkingByLength', 'chunkingByCharacter', 'chunkingAuto', 
            'retrieving', 'convert2text', 'convert2structured', 'editText', 
            'editStructured', 'searchGoogle', 'searchPerplexity', 'llmnew', 
            'ifelse', 'generate', 'load'
        ],
        servernode: ['server'],
        groupnode: ['group']
    };
    ...
    // 根据类别过滤节点
    const filterNodesByCategory = useCallback((nodeIds: string[], category: NodeCategory) => {
    ...
    const getSourceNodeIdWithLabel = useCallback((parentId: string, category: NodeCategory = 'all') => {
    ...
    const getTargetNodeIdWithLabel = useCallback((parentId: string, category: NodeCategory = 'all') => {
