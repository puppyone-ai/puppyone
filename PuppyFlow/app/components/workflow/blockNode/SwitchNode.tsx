...
type SwitchNodeProps = NodeProps<Node<SwitchNodeData>>

function SwitchNode({isConnectable, id, type, data: {content, label, isLoading, locked, isInput, isOutput, editable}}: SwitchNodeProps ) {
  ...
  const onSwitchButtonClick = () => {
    ...
    setTimeout(() => {
      setNodes(nodes => nodes.map(node => 
        node.id === id ? { ...node, data: { ...node.data, content: content === "ON" ? "OFF" : "ON" } } : node
      ));
