...
function JsonNodeSettingMenu({
  showSettingMenu,
  clearMenu,
  nodeid,
}: JsonNodeSettingMenuProps) {
  ...
  const constructMetadataInfo = (
    data: any,
    embeddingViewData: EmbeddingItem[]
  ) => {
  ...
  const onEmbeddingClick = async () => {
    ...
    try {
      ...
      const embeddingViewDataWithInfo = constructMetadataInfo(
        embeddingNodeData.data.content,
        embeddingViewData
      );
      ...
      setNodes(prevNodes =>
        prevNodes.map(node => {
          if (node.id === nodeid) {
            return {
              ...node,
              data: { ...node.data, chunks: embeddingViewDataWithInfo },
            };
          }
          return node;
        })
      ...
      const response = await fetch(
        `${PuppyStorage_IP_address_for_embedding}/Rose123`,
        {
          method: 'POST',
          headers: {
  ...
  const constructStructuredNodeEmbeddingData = () => {
    ...
    const nodeContent =
      ...
      (node?.type === 'none' && node?.data?.subType === 'structured')
        ? cleanJsonString(node?.data.content as string | any)
  ...
  return (
    ...
    >
      <ul className='flex flex-col absolute top-[8px] p-[8px] w-[240px] gap-[4px] bg-[#252525] border-[1px] border-[#404040] rounded-[8px] left-0 z-[20000]'>
        {/* <li>
        ...
        <li className='w-full h-[1px] bg-[#404040] my-[2px]'></li>
        {/* External storage for structured block */}
        <li>
          <button
