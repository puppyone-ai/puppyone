...
function FileNodeSettingMenu({
  showSettingMenu,
  clearMenu,
  nodeid,
}: FileNodeSettingMenuProps) {
  const {
    manageNodeasLocked,
    setNodeEditable,
  ...
  const [tempUploadInfo, setTempUploadInfo] = useState<{
    uploadUrl: string;
    uploadId: string;
  } | null>(null);
  ...
  const handleInputChange = async (
    ...
    try {
      ...
      const response = await fetch(
        `${PuppyStorage_IP_address_for_uploadingFile}/Rose123`
      );

      if (!response.ok) {
        throw new Error(
          `Fetch temporary upload info Error: ${response.status}`
        );
      }
          ...
          <li>
            ...
            >
              <div className='flex items-center justify-center'>
                ...
                >
                  <path
                    d='M8 15L8 9L19 9L19 15'
                    stroke='currentColor'
                    strokeWidth='2'
                  />
                  <path d='M13.5 21V14' stroke='currentColor' strokeWidth='2' />
                  <path
                    d='M13.5 11L16.5 15H10.5L13.5 11Z'
                    fill='currentColor'
                  />
                </svg>
              </div>
