...
export type VectorIndexingStatus =
  | 'notStarted'
  | 'processing'
  | 'done'
  | 'error'
    ...
    async (
      ...
      if (newItem.type === 'vector') {
        ...
        for (let i = 0; i < dataSource.length; i++) {
          ...
          const indexContent = getValueByPath(
            source,
            (newItem as VectorIndexingItem).key_path
          );

          // 获取值路径结果作为原始 metadata 内容
          const retrievalContent = getValueByPath(
            source,
            (newItem as VectorIndexingItem).value_path
          );
        ...
        try {
          ...
          if (indexNameResponse.collection_name) {
            // 更新向量索引项的信息
            (newItem as VectorIndexingItem).index_name =
              indexNameResponse.collection_name;
            ...
            console.log(
              'Updated vector indexing with collection name:',
              indexNameResponse.collection_name
            );
          }
        } catch (error) {
      ...
      if (itemToRemove && itemToRemove.type === 'vector') {
        ...
        try {
          ...
          const deleteParams = {
            ...
            user_id: userId,
            model:
              vectorItem.collection_configs.model || 'text-embedding-ada-002',
