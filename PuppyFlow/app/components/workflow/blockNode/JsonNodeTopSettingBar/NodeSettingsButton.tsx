...
function NodeSettingsController({ nodeid }: NodeSettingsControllerProps) {
    ...
    const { activatedNode, setHandleActivated, manageNodeasLocked, setNodeEditable, preventInactivateNode, clearAll } = useNodesPerFlowContext()
    ...
    return (
        <div ref={componentRef} style={{ position: 'relative', isolation: 'isolate' }}>
            <button ref={settingControllerRef} className={`flex items-center justify-center ${isHovered || isMenuOpen ? "bg-[#3E3E41]" : ""} w-[24px] h-[24px] rounded-[8px]`} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} onClick={manageSettings}>
