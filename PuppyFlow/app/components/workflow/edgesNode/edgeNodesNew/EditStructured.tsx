...
const EditStructured: React.FC<ModifyConfigNodeProps> = React.memo(
  ({ data, isConnectable, id }) => {
    ...
    const [variableReplacePlugins, setVariableReplacePlugins] = useState<{
      ...
      () =>
        (getNode(id)?.data.variableReplacePlugins as {
          [key: string]: string;
        }) || {}
    );
    ...
    const [paramv, setParamv] = useState(
      () => (getNode(id)?.data.paramv as string) || ''
    );
          ...
          if (node.id === id) {
            return {
              ...
              data: {
                ...
                variableReplacePlugins: variableReplacePlugins,
              },
            };
          }
          return node;
            ...
            {execMode === MODIFY_APPLY_TEMPLATE && (
              <>
                ...
                <li className='flex flex-col gap-[8px]'>
                  ...
                  <div className='flex flex-col gap-2 p-3 bg-[#252525] rounded-[8px] border-[1px] border-[#6D7177]/30'>
                        ...
                        <div key={index} className='flex gap-2 items-center'>
                          <input
                            ...
                            onChange={e => {
                              ...
                              newPlugins[e.target.value] = value as string;
                              setVariableReplacePlugins(newPlugins);
                            }}
                            placeholder='Key'
                            className='flex-1 h-[28px] px-2 bg-[#1A1A1A] border-[1px] border-[#6D7177]/30 rounded-[6px] text-[#CDCDCD] text-[11px] placeholder-[#6D7177] outline-none focus:border-[#6D7177]/50'
                            onFocus={onFocus}
                            onBlur={onBlur}
                          />
                    ...
                    <button
                      ...
                      className='w-full h-[28px] flex items-center justify-center gap-2 rounded-[6px] 
                                 border border-[#6D7177]/30 bg-[#1A1A1A] text-[#CDCDCD] text-[11px] font-medium 
                                 hover:border-[#6D7177]/50 hover:bg-[#252525] transition-colors'
                    >
                      <svg
                        width='12'
                        height='12'
