...
function getGroupBlockNodes(groupNodeId: string, context: RunGroupNodeContext) {
  ...
  console.log(
    `📊 [getGroupBlockNodes] 组内BlockNode数量: ${groupBlockNodes.length}`
  );
...
function collectAllRelatedEdgeNodes(
  blockNodes: { id: string; label: string }[],
  context: RunGroupNodeContext
) {
  console.log(
    `🔗 [collectAllRelatedEdgeNodes] 开始执行 - 处理${blockNodes.length}个block nodes`
  );
  ...
  blockNodes.forEach(blockNode => {
    console.log(
      `🔗 [collectAllRelatedEdgeNodes] 处理blockNode: ${blockNode.id}`
    );

    // 获取每个blockNode的源节点（连入该block的edge nodes）
    const sourceNodes = context.getSourceNodeIdWithLabel(
      blockNode.id,
      'edgenode'
    );
    console.log(
      `🔗 [collectAllRelatedEdgeNodes] ${blockNode.id} 的源节点数量: ${sourceNodes.length}`
    );
    ...
    const targetNodes = context.getTargetNodeIdWithLabel(
      blockNode.id,
      'edgenode'
    );
    console.log(
      `🔗 [collectAllRelatedEdgeNodes] ${blockNode.id} 的目标节点数量: ${targetNodes.length}`
    );
  ...
  console.log(
    `🔗 [collectAllRelatedEdgeNodes] 执行完成，找到${result.length}个edge nodes`
  );
  return result;
}
...
function filterValidEdgeNodes(
  ...
  console.log(
    `✅ [filterValidEdgeNodes] 开始执行 - 处理${edgeNodeIds.length}个edge nodes`
  );
  ...
  edgeNodeIds.forEach(edgeNodeId => {
    ...
    const inputNodes = context.getSourceNodeIdWithLabel(
      edgeNodeId,
      'blocknode'
    );
    ...
    const outputNodes = context.getTargetNodeIdWithLabel(
      edgeNodeId,
      'blocknode'
    );
  ...
  console.log(
    `✅ [filterValidEdgeNodes] 执行完成，${validEdgeNodes.length}个有效edge nodes`
  );
  return validEdgeNodes;
}

// 步骤3: 根据确定好的要提交到后端的edgenode，找到所有input和output的blocknode（无论在不在group里面），然后剔除相同的
function collectAllRelatedBlockNodes(
  validEdgeNodeIds: string[],
  context: RunGroupNodeContext
) {
  console.log(
    `📦 [collectAllRelatedBlockNodes] 开始执行 - 处理${validEdgeNodeIds.length}个有效edge nodes`
  );
...
// 构建包含所有相关节点的JSON数据
function constructGroupNodeJson(
  groupNodeId: string,
  context: RunGroupNodeContext,
  ...
  try {
    ...
    // 步骤1: 根据组内的blocknode找到它的input和output的edgenode
    const allRelatedEdgeNodeIds = collectAllRelatedEdgeNodes(
      groupBlockNodes,
      context
    );
    ...
    const allRelatedBlockNodeIds = collectAllRelatedBlockNodes(
      validEdgeNodeIds,
      context
    );
    ...
    allRelatedBlockNodeIds.forEach(blockNodeId => {
      ...
      if (blockNodeTypes.includes(node.type || '')) {
        try {
          ...
          console.log(
            `✅ [constructGroupNodeJson] 成功构建block node: ${blockNodeId}`
          );
        } catch (e) {
    ...
    validEdgeNodeIds.forEach(edgeNodeId => {
      ...
      try {
        ...
        console.log(
          `✅ [constructGroupNodeJson] 成功构建edge node: ${edgeNodeId}`
        );
      } catch (e) {
    ...
    Object.entries(edges).forEach(([edgeId, edgeData]) => {
      ...
      if (existingEdgeId) {
        console.log(
          `🔄 发现重复的边节点: ${edgeId} 与 ${existingEdgeId} 相同，删除 ${edgeId}`
        );
...
async function sendGroupDataToTargets(
  groupNodeId: string,
  context: RunGroupNodeContext,
  customConstructJsonData?: () => BaseConstructedJsonData
): Promise<void> {
  ...
  const jsonData = constructGroupNodeJson(
    groupNodeId,
    context,
  ...
  try {
    ...
    if (!response.ok) {
      outputNodeIds.forEach(nodeId => {
        if (nodeId)
          context.reportError(nodeId, `HTTP Error: ${response.status}`);
      });
      return;
    }
    ...
    if (result && result.task_id) {
      console.log(
        `🔄 [sendGroupDataToTargets] 开始流式处理，task_id: ${result.task_id}`
      );
...
export function buildGroupNodeJson({
  ...
  console.log(
    `🔧 [buildGroupNodeJson] 开始构建JSON数据 - groupNodeId: ${groupNodeId}`
  );
