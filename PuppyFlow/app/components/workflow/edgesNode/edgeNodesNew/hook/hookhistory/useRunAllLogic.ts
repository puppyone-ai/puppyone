...
export function useBaseEdgeNodeLogic({
    ...
    // 执行流程
    useEffect(() => {
        console.log(`🔄 [useRunAllLogic - useEffect] 执行 - isComplete: ${isComplete}`);
    ...
    const sendDataToTargets = async () => {
        console.log(`🚀 [useRunAllLogic - sendDataToTargets] 开始发送数据到目标节点`);
        
        // 获取所有节点
        const allNodes = getNodes();
        console.log(`📊 [useRunAllLogic - sendDataToTargets] 获取所有节点数量: ${allNodes.length}`);
        ...
        console.log(`📊 [useRunAllLogic - sendDataToTargets] 找到${resultNodes.length}个结果节点需要设置为加载状态`);
        ...
        try {
            ...
            if (!response.ok) {
                console.error(`❌ [useRunAllLogic - sendDataToTargets] HTTP请求失败: ${response.status}`);
            ...
            if (result && result.task_id) {
                console.log(`🔄 [useRunAllLogic - sendDataToTargets] 开始流式处理，task_id: ${result.task_id}`);
                ...
                console.log(`📊 [useRunAllLogic - sendDataToTargets] 准备流式处理${resultNodes.length}个结果节点`);
            ...
            console.log(`📊 [useRunAllLogic - sendDataToTargets] 重置${nodesToReset.length}个节点的加载UI`);
            ...
            // 添加回调
            if (onComplete) {
                console.log(`🔄 [useRunAllLogic - sendDataToTargets] 调用onComplete回调`);
                onComplete();
            }
        }
    };
    ...
    const constructAllNodesJson = (): BaseConstructedJsonData => {
        console.log(`🔧 [useRunAllLogic - constructAllNodesJson] 开始构建所有节点的JSON数据`);
        ...
        try {
            ...
            console.log(`📊 [useRunAllLogic - constructAllNodesJson] 所有节点数量: ${allNodes.length}, 边数量: ${reactFlowEdges.length}`);
            ...
            allNodes.forEach(node => {
                ...
                console.log(`🔧 [useRunAllLogic - constructAllNodesJson] 处理节点: ${nodeId}, 类型: ${node.type}`);
                
                // 根据节点类型决定如何构建JSON
                if (blockNodeTypes.includes(node.type || '')) {
                    console.log(`📦 [useRunAllLogic - constructAllNodesJson] 构建block节点: ${nodeId}`);
                    ...
                    try {
                        ...
                        console.log(`✅ [useRunAllLogic - constructAllNodesJson] 成功构建block节点: ${nodeId}`);
                    } catch (e) {
                    ...
                    console.log(`🔗 [useRunAllLogic - constructAllNodesJson] 构建edge节点: ${nodeId}`);
                    ...
                    try {
                        ...
                        console.log(`✅ [useRunAllLogic - constructAllNodesJson] 成功构建edge节点: ${nodeId}`);
                    } catch (e) {
            ...
            console.log(`🚀 [useRunAllLogic - constructAllNodesJson] 构建完成 - blocks: ${Object.keys(blocks).length}, edges: ${Object.keys(edges).length}`);
    ...
    const handleDataSubmit = async (...args: any[]) => {
        console.log(`🚀 [useRunAllLogic - handleDataSubmit] 开始处理数据提交 - isComplete: ${isComplete}`);
        ...
        try {
            ...
            console.log(`🔄 [useRunAllLogic - handleDataSubmit] 设置isComplete为false，触发useEffect`);
            setIsComplete(false);
        } catch (error) {
