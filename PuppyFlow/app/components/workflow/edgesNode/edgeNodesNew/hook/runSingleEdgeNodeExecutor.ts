...
// 创建新的目标节点
async function createNewTargetNode(parentId: string, context: RunSingleEdgeNodeContext): Promise<void> {
    console.log(`🔧 [createNewTargetNode] 开始创建新的目标节点 - parentId: ${parentId}`);
...
async function sendDataToTargets(
    ...
    console.log(`🚀 [sendDataToTargets] 开始发送数据到目标节点 - parentId: ${parentId}`);
    
    const targetNodeIdWithLabelGroup = context.getTargetNodeIdWithLabel(parentId);
    console.log(`📊 [sendDataToTargets] 找到${targetNodeIdWithLabelGroup.length}个目标节点`);
...
// 默认构建 JSON 数据
function defaultConstructJsonData(parentId: string, context: RunSingleEdgeNodeContext): BaseConstructedJsonData {
    console.log(`🔧 [defaultConstructJsonData] 开始构建默认JSON数据 - parentId: ${parentId}`);
    
    const sourceNodeIdWithLabelGroup = context.getSourceNodeIdWithLabel(parentId, 'blocknode');
    const targetNodeIdWithLabelGroup = context.getTargetNodeIdWithLabel(parentId, 'blocknode');
    
    console.log(`📊 [defaultConstructJsonData] 源节点数: ${sourceNodeIdWithLabelGroup.length}, 目标节点数: ${targetNodeIdWithLabelGroup.length}`);
...
export async function runSingleEdgeNode({
    ...
    try {
        ...
        const targetNodeIdWithLabelGroup = context.getTargetNodeIdWithLabel(parentId);
        console.log(`📊 [runSingleEdgeNode] 找到${targetNodeIdWithLabelGroup.length}个目标节点`);
