...
export function useRunGroupNodeLogic({
    ...
    const getGroupEdgeNodes = () => {
        ...
        const edgeNodeTypes = [
            ...
            'searchGoogle', 'searchPerplexity', 'llmnew', 'ifelse', 'generate', 'load'
        ];

        // 筛选出组内的 edgeNodes - 修改为检查 groupIds 数组
        const groupEdgeNodes = allNodes.filter(node => {
            const groupIds = (node.data as any)?.groupIds;
            const isInGroup = Array.isArray(groupIds) && groupIds.includes(groupNodeId);
            const isEdgeNode = edgeNodeTypes.includes(node.type || '');
            return isInGroup && isEdgeNode;
        });

        return groupEdgeNodes.map(node => ({
            id: node.id,
    ...
    const constructGroupNodeJson = (): BaseConstructedJsonData => {
        try {
            // 获取组内所有 edge nodes
            const groupEdgeNodes = getGroupEdgeNodes();

            if (groupEdgeNodes.length === 0) {
                console.warn('没有找到组内的EdgeNode');
                return { blocks: {}, edges: {} };
            }

            // 收集所有相关的block nodes
            const allRelatedBlockNodeIds = collectAllRelatedBlockNodes(groupEdgeNodes);
            ...
            // 构建所有组内 edge nodes的JSON
            groupEdgeNodes.forEach(edgeNode => {
                try {
