...
type MainMenuItem = {
  ...
  buildSubmenu?: () => Array<{ key: string; label: string; onPickEdgeType?: string; disabled?: boolean; subKey?: string }>; // submenu entries
};
type SubItem = { key: string; label: string; onPickEdgeType?: string; disabled?: boolean };
type ActionItem = { key: string; label: string; onPickEdgeType?: string; submenuKey?: 'chunk' | 'retrieve' | 'modify' | 'search' };
...
const EdgeMenuNode: React.FC<EdgeMenuTempNodeProps> = ({ id, data, isConnectable }) => {
  ...
  const buildSections = useCallback((): MenuSection[] => {
    if (sourceType === 'file') {
      return [
        { key: 'load-section', label: 'Load', items: [{ key: 'load', label: 'Load', onPickEdgeType: 'load' }] },
      ];
    }
    ...
    const sections: MenuSection[] = [
      ...
      // Moved Generate into RAG, removed separate Generation section
      {
        ...
        label: 'Deep Research',
        items: [
          { key: 'deepresearch', label: 'Deep Research', onPickEdgeType: 'deepresearch' },
        ],
      },
      {
        key: 'search',
        label: 'Searching',
        items: [
          { key: 'search', label: 'Search ▸', submenuKey: 'search' },
        ],
      },
      {
        key: 'other',
  ...
  const chunkSubmenu = useMemo(
    (): SubItem[] => [
      ...
      { key: 'by-length', label: 'By length', onPickEdgeType: 'chunkingByLength' },
      { key: 'by-character', label: 'By character', onPickEdgeType: 'chunkingByCharacter' },
    ],
    []
  );
    ...
    (): SubItem[] =>
      sourceType === 'text'
        ? [
            ...
            { key: 'convert-structured', label: 'Convert → structured', onPickEdgeType: 'convert2structured' },
            { key: 'edit-text', label: 'Edit (text)', onPickEdgeType: 'editText' },
          ]
        : [
            { key: 'copy', label: 'Copy', onPickEdgeType: 'copy' },
            { key: 'convert-text', label: 'Convert to Text', onPickEdgeType: 'convert2text' },
            { key: 'edit-structured', label: 'Edit (structure)', onPickEdgeType: 'editStructured' },
          ],
    [sourceType]
  );
            ...
            {sections.map((section) => (
              <React.Fragment key={`section-${section.key}`}>
                ...
                {section.items.map((item) => {
                  ...
                  const isDisabled = !item.onPickEdgeType && !item.submenuKey && item.key.includes('placeholder');
                  return (
                    <li
                      key={item.key}
                      ref={(el) => {
                      ...
                      <div className='flex items-center gap-[11px] flex-1'>
                        ...
                        <div className='w-[30px] h-[30px] bg-[#1C1D1F] flex items-center justify-center rounded-[5px]'>
                          ...
                          {item.key === 'deepresearch' && (
                            <svg width='14' height='14' viewBox='0 0 14 14' fill='none' xmlns='http://www.w3.org/2000/svg'>
                              <path d='M7 2.5C7 2.5 4.5 1 2 3.5C2 3.5 1.5 6.5 4 7.5C4 7.5 6 8.5 7 11.5C7 11.5 8 8.5 10 7.5C10 7.5 12.5 6.5 12 3.5C12 3.5 9.5 1 7 2.5Z' stroke='#CDCDCD' strokeWidth='1.5' fill='none'/>
                              <circle cx='7' cy='7' r='2' stroke='#CDCDCD' strokeWidth='1.5' fill='none'/>
                            </svg>
                          )}
                          {item.key === 'search' && (
                            <svg width='14' height='14' viewBox='0 0 14 14' fill='none' xmlns='http://www.w3.org/2000/svg'>
                    ...
                    >
                      <div className='flex items-center gap-[11px] flex-1'>
                        <div className='w-[30px] h-[30px] bg-[#1C1D1F] flex items-center justify-center rounded-[5px]'>
                          ...
                          {(s.key === 'convert-structured' || s.key === 'convert-text') && (
                          ...
                          {(s.key === 'edit-text' || s.key === 'edit-structured') && (
                            <svg xmlns='http://www.w3.org/2000/svg' width='14' height='14' viewBox='0 0 14 14' fill='none'>
                          ...
                          {(s.key === 'for-html' || s.key === 'for-markdown') && (
