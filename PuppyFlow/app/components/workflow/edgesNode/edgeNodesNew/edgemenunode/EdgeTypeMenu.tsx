        ...
        items: [
          {
            ...
            label: 'LLM',
            description: 'with AI',
            onPickEdgeType: 'llmnew',
          },
          {
            key: 'modify',
            label: 'Modify',
            description: 'Copy, convert, edit',
            submenu: [],
          },
        ],
      },
      {
        ...
        items: [
          ...
          },
          {
            key: 'retrieve',
            label: 'Retrieve',
            description: 'by query from a base',
            onPickEdgeType: 'retrieving',
          },
          {
            key: 'generate',
            label: 'Generate',
            description: 'Generate with context',
            onPickEdgeType: 'generate',
          },
        ],
      },
      {
...
const ELEMENT_REGISTRY: ElementRegistry = {
  ...
  weblink: {
    key: 'weblink',
    level: 'section',
    description: 'Web Link (not available yet)',
    icon: () => <></>,
  },
  ...
  google: {
    ...
    level: 'submenu',
    description: 'Google',
    icon: () => <FontAwesomeIcon icon={faGoogle} color='#CDCDCD' size='sm' />,
  },
  perplexity: {
    key: 'perplexity',
    level: 'submenu',
    ...
    >
      <div className='flex items-center gap-[11px] flex-1'>
        ...
        <div className='flex flex-col items-start justify-center'>
          <div className='menu-item__label text-[12px] font-plus-jakarta-sans leading-[16px] text-[#CDCDCD] group-hover:text-black transition-colors duration-100 ease-out'>
            {item.label}
          </div>
          {item.description && (
      ...
      {hasSubmenu && (
        <span className='text-[#9AA0A6] group-hover:text-black transition-colors duration-100 ease-out'>
          ...
          >
            <path
              d='M6 10.2427L10.2426 6.00004L6 1.75739'
              stroke='currentColor'
            />
          </svg>
        </span>
          ...
          <li className='text-left w-full h-[18px] text-[#6D7177] text-[10px] tracking-wide font-normal flex items-center px-[4px]'>
            {section.label
              ? section.label.charAt(0).toUpperCase() +
                section.label.slice(1).toLowerCase()
              : ''}
          </li>
        ...
        {items.map((s, si) => (
          ...
          >
            <div className='flex items-center gap-[11px] flex-1'>
              ...
              <div className='flex flex-col items-start justify-center'>
                <div className='submenu-item__label text-[12px] font-plus-jakarta-sans leading-[16px] text-[#CDCDCD] group-hover:text-black transition-colors duration-100 ease-out'>
                  {s.label}
                </div>
              </div>
            </div>
...
const EdgeTypeMenu: React.FC<EdgeTypeMenuProps> = ({
  ...
  const sections = useMemo(
    () => getMenuSectionsFromConfig(sourceType),
    [sourceType]
  );
      ...
      >
        ...
        {openSubmenuIndex !== null && (
          <SubmenuPanel
            ...
            items={
              openSubmenuIndex !== null
                ? (flatItems[openSubmenuIndex]?.submenu ?? [])
                : []
            }
            onMouseEnter={() => {
              if (openSubmenuIndex !== null)
                openSubmenuWithDelay(openSubmenuIndex);
            }}
