...
function OthersSubMenu({nodeType, sourceNodeId, showMenu}: OthersSubMenuProps) {
    
    const topPosition = nodeType === "text" ? "top-[250px]" :
                        nodeType === "structured" ? "top-[285px]" :
                        "top-[250px]"
  ...
  return (
    <ul id="edgeMenu" className={`bg-[#1c1d1f] rounded-[11px] border-solid border-[3px] border-[#42454A] absolute float-start flex flex-col justify-evenly z-[20001] ${topPosition} left-[135px] gap-[3px] p-[3px] items-center ${showMenu === 1 ? "" : "hidden"}`} >
            ...
            <li className='w-full'>
                ...
                <path fill="#1C1D1F" stroke="#CDCDCD" strokeWidth="1.5" d="M.75.75h10.5v6.5H.75z"/>
                <path fill="#CDCDCD" d="m7 8-1 4-1-4h2Z"/>
                </svg>
                </div>
                <div className='text-[12px] font-[700] flex items-center justify-center h-full'>generating</div>
                </button>
            </li> 
            </ul>
  )
