...
function LoadConfigMenu({ show, parentId }: LoadConfigProps) {
    ...
    const [resultNode, setResultNode] = useState<string | null>(
        (getNode(parentId)?.data as ModifyConfigNodeData)?.resultNode ?? null
    )
    ...
    const constructJsonData = (): ConstructedLoadJsonData => {
        const sourceNodeIdWithLabelGroup = getSourceNodeIdWithLabel(parentId)
        const targetNodeIdWithLabelGroup = getTargetNodeIdWithLabel(parentId)
        ...
        console.log("Load Config JSON Data:", { blocks, edges })
