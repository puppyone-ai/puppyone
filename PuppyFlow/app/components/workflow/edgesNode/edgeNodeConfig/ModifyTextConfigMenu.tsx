...
function ModifyTextConfigMenu({ show, parentId }: ModifyTextConfigProps) {
    ...
    const constructJsonData = (): ConstructedModifyTextJsonData => {
        const sourceNodeIdWithLabelGroup = getSourceNodeIdWithLabel(parentId);
        const targetNodeIdWithLabelGroup = getTargetNodeIdWithLabel(parentId);
        ...
        console.log("ModifyText JSON Data:", { blocks, edges });
    ...
    const [retMode, setRetMode] = useState(typeof (getNode(parentId)?.data?.extra_configs as any)?.retMode === 'string' ? (getNode(parentId)?.data?.extra_configs as any)?.retMode : RET_ALL)
