...
function Modify2StructuredConfigMenu({ show, parentId }: ModifyCopyConfigProps) {
    ...
    const constructJsonData = (): ConstructedModifyCopyJsonData => {
        ...
        const edgejson: Modify2SturcturedJsonType = {
            ...
            data: {
                ...
                extra_configs: {
                    ...
                    action_type: execMode === JSON_TYPE ? "json" : "default",
                    ...(execMode === BY_CHAR_TYPE ? { list_separator: JSON.parse(deliminator) } : {}),
                    ...(execMode === INTO_DICT_TYPE ? { dict_key: wrapInto } : {}),
                    ...(execMode === BY_LEN_TYPE ? { length_separator: bylen } : {})
                },
        ...
        console.log("Modify2Structured JSON Data:", { blocks, edges });
