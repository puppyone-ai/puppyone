...
import ChangeableEditor from '../../menu/tableComponent/ChangeableEditor'
import JsonBlockNode from './JsonNode'
import TextEditorTextArea from '../../menu/tableComponent/TextEditorTextArea'



type ResultBlockNodeProps = NodeProps<Node<{ content: string, subtype: "text" | "structured", label: string }>>

function ResultBlockNode({data: {content, subtype = "text", label}, isConnectable, id, type}: ResultBlockNodeProps ) {
  ...
  return (
    <div ref={componentRef} className="relative w-full h-full min-w-[240px] min-h-[304px] p-[32px] ">
      <div ref={contentRef} id={id} className={`w-full h-full border-[1.5px] min-h-[240px] rounded-[8px] p-[16px] ${getBorderColor()} text-[#CDCDCD] bg-main-black-theme break-words font-plus-jakarta-sans text-base leading-5 font-[400] overflow-hidden`}  >
