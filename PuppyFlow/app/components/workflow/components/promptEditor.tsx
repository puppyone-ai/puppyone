...
const getHighlightedContent = (content: string, variables: { name: string; type?: string }[]) => {
    return content
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
        .replace(/\{\{([^{}]+)\}\}/g, (match, label) => {
            // 查找匹配的节点及其类型
            const sourceNode = variables.find(item => item.name.trim() === label.trim());
...
const PromptEditor: React.FC<PromptEditorProps> = ({
    ...
    useEffect(() => {
        // 检查内容是否真的变化了
        const isSameContent = messagesRef.current.length === messages.length &&
    ...
    useEffect(() => {
        ...
        if (!isEditingRef.current) {
            ...
            timeoutRef.current = setTimeout(() => {
                const updatedMessages = prompts.map(({ role, content }) => ({ role, content }));
                
                // 检查是否有实际变化
                const hasChanged = updatedMessages.length !== messagesRef.current.length ||
                    updatedMessages.some((msg, i) => {
                        const oldMsg = messagesRef.current[i];
                        return !oldMsg || msg.role !== oldMsg.role || msg.content !== oldMsg.content;
                    });
