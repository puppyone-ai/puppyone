...
const AdvancedPathEditor: React.FC<AdvancedPathEditorProps> = ({ path, onChange }) => {
  ...
  return (
    <div className='flex flex-col gap-2'>
      {segments.length === 0 ? (
        ...
        >
          <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#6D7177">
        ...
        <>
          {segments.map((segment, index) => (
            <div key={segment.id} className="flex items-center gap-[4px] relative">
              ...
              <div className="flex-1 relative h-[28px] bg-[#252525] rounded-[6px] border-[1px] border-[#6D7177]/30 hover:border-[#6D7177]/50 transition-colors overflow-hidden">
                <input
                  ...
                  placeholder={segment.type === 'num' ? 'Enter number...' : 'Enter key...'}
                />
                ...
                >
                  <div className={`text-[10px] min-w-[24px] text-center
                              ${segment.type === 'key'
                    ? 'text-[#9B6DFF]'
                    : 'text-[#FF9B4D]'}`}>
                    {segment.type}
