...
const ServerDisplayContent: React.FC = () => {
  const { currentServiceJson, currentShowingId, isLoading } = useServers();

  const { isValidApiService, isValidChatbotService } =
    useServiceValidation(currentServiceJson);
    ...
    return (
      <div className='w-full h-full flex items-center justify-center bg-[#131313]'>
        <div className='flex flex-col items-center space-y-4'>
          <svg
            className='animate-spin w-8 h-8 text-[#4599DF]'
            fill='none'
            viewBox='0 0 24 24'
          >
            <circle
      ...
      <div className='w-full h-full flex items-center justify-center bg-[#131313]'>
        <div className='text-center'>
          ...
          <div className='text-[#888888] text-sm'>
            Please select a service from the sidebar
          </div>
        </div>
      </div>
    );
