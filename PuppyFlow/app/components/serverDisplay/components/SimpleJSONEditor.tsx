...
const SimpleJSONEditor: React.FC<SimpleJSONEditorProps> = ({
  ...
  const generateLineNumbers = () => {
    ...
    for (let i = 1; i <= lineCount; i++) {
      numbers.push(
        <div key={i} className="text-[#6D7177] text-sm leading-6 text-right pr-2 select-none">
          {i}
        </div>
      );
    }
  ...
  return (
    <div className={`relative flex flex-col border-none rounded-[8px] cursor-pointer pl-[2px] pt-[8px] bg-[#1C1D1F] h-full ${className}`} style={style}>
      {/* 占位符 - 参考 JSONForm 的样式 */}
      {isEmpty && (
