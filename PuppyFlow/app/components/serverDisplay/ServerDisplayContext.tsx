...
interface ServerDisplayContextType {
  ...
  updateApiServiceState: (
    apiId: string,
    updates: Partial<ApiServiceState>
  ) => void;
  getCurrentApiServiceState: (apiId: string) => ApiServiceState;
}

// 创建Context
const ServerDisplayContext = createContext<
  ServerDisplayContextType | undefined
>(undefined);
...
export const ServerDisplayProvider: React.FC<ServerDisplayProviderProps> = ({
  children,
}) => {
  ...
  const [apiServiceStates, setApiServiceStates] = useState<
    Record<string, ApiServiceState>
  >({});

  // 获取当前API服务的状态
  const getCurrentApiServiceState = useCallback(
    (apiId: string): ApiServiceState => {
  ...
  // 更新API服务状态
  const updateApiServiceState = useCallback(
    (apiId: string, updates: Partial<ApiServiceState>) => {
  ...
  const updateChatHistory = useCallback(
    (chatbotId: string, newMessage: ChatMessage) => {
...
export const useServerDisplayContext = () => {
  ...
  if (context === undefined) {
    throw new Error(
      'useServerDisplayContext must be used within a ServerDisplayProvider'
    );
  }
  return context;
};
