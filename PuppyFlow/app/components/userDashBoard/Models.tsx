...
const Models: React.FC = () => {
  ...
  const groupedModels = useMemo(() => {
    const allLLMs = [
      ...localModels.filter(model => model.type !== 'embedding').map(model => ({ ...model, deployment: 'local' })),
      ...cloudModels.filter(model => model.type !== 'embedding').map(model => ({ ...model, deployment: 'cloud' }))
    ];
  ...
  // 渲染模型列表的通用组件
  const ModelList = ({ models, emptyMessage }: { models: any[], emptyMessage: string }) => (
    models.length > 0 ? (
      ...
      <div className="py-[8px] space-y-6 overflow-y-auto">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h4 className="text-[16px] font-medium text-[#AAAAAA]">LLM Models</h4>
            <button 
        ...
        <div className="space-y-4">
          <h4 className="text-[16px] font-medium text-[#AAAAAA]">Embedding Models</h4>
