...
function AddNewWorkspaceButton() {
  const { workspaceManagement, addWorkspace, createEmptyWorkspace, setShowingWorkspace } = useWorkspaces()
  ...
  const handleAddFlow = async () => {
    try {
      ...
      const newWorkspace = createEmptyWorkspace(newWorkspaceId, newWorkspaceName)
      ...
      // 异步创建工作区到数据库
      const createdWorkspace = await workspaceManagement.createWorkspace(newWorkspaceId, newWorkspaceName)
    ...
    <div className="flex h-[32px] items-center mt-[24px] relative self-stretch w-full cursor-pointer">
      ...
      >
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" className="group-hover:[&>*]:stroke-[#CDCDCD]">
          <rect x="0.75" y="0.75" width="14.5" height="14.5" rx="3.25" stroke="#5D6065" strokeWidth="1.5"/>
          <path d="M8 4V12" stroke="#5D6065" strokeWidth="1.5"/>
          <path d="M4 8L12 8" stroke="#5D6065" strokeWidth="1.5"/>
        </svg>
        <span className='text-[12px] font-medium text-[#5D6065] group-hover:text-[#CDCDCD]'>New</span>
      </button>
    </div>
  )
}
