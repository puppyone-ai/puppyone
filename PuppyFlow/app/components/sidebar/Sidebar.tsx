...
function SidebarHidden({ setFlowFullScreen }: SidebarHiddenProps) {
  ...
  useEffect(() => {
    const handleClick = (event: MouseEvent) => {
      ...
      const FlowOutlineMenuButton = document.getElementById(
        'FlowOutlineMenuButton'
      );
      const FlowOutlineMenuGroups = Array.from(
        document.getElementsByClassName(
          'FlowOutlineMenuGroup'
        ) as HTMLCollection
      );
      const isOutside =
        FlowOutlineMenuGroups &&
        !FlowOutlineMenuGroups.some(group =>
      ...
      if (isOutside) {
        ...
        FlowOutlineMenuButton &&
        FlowOutlineMenuButton.contains(event.target as Node)
      ) {
  ...
  return (
    <div className='w-[64px] h-screen bg-[#252525] flex flex-col items-center pt-[16px] gap-[16px] transition-all duration-300 ease-in-out relative'>
      ...
      <button
        id='FlowOutlineMenuButton'
        className='w-[32px] h-[32px] flex items-center justify-center group FlowOutlineMenuGroup transition-all duration-200'
      >
        <svg
          width='32'
          height='32'
        ...
        >
          ...
          <path
            fillRule='evenodd'
            clipRule='evenodd'
            d='M13.1 18.9996C13.5633 21.2818 15.5811 22.9996 18 22.9996C20.7614 22.9996 23 20.761 23 17.9996C23 15.5806 21.2823 13.5629 19 13.0996V14.6445C20.4458 15.0748 21.5 16.4141 21.5 17.9996C21.5 19.9326 19.933 21.4996 18 21.4996C16.4145 21.4996 15.0752 20.4453 14.645 18.9996H13.1Z'
            className='fill-[#8B8B8B] group-hover:fill-[#FFFFFF]'
          />
        </svg>
      </button>
