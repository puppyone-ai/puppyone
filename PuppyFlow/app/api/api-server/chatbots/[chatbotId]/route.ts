...
export async function DELETE(_: NextRequest, context: { params: { chatbotId: string } }) {
  const apiBase = SYSTEM_URLS.API_SERVER.BASE;
  const { chatbotId } = context.params;
  const upstream = await fetch(`${apiBase}/delete_chatbot/${encodeURIComponent(chatbotId)}`, {
    method: 'DELETE',
...
export async function PUT(req: NextRequest, context: { params: { chatbotId: string } }) {
  const apiBase = SYSTEM_URLS.API_SERVER.BASE;
  const { chatbotId } = context.params;
  let body: any = {};
  try { body = await req.json(); } catch {}
  const upstream = await fetch(`${apiBase}/update_chatbot/${encodeURIComponent(chatbotId)}`, {
    method: 'PUT',
    headers: buildUserHeaders(),
