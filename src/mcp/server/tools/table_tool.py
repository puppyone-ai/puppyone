...
class TableTool:
    ...
    def generate_tool_description(
        ...
        Args:
            project_name: 项目名称
            table_name: 表格名称
            tool_type: 工具类型（create/update/delete/query）
            project_description: 项目描述
            project_metadata: 项目元数据
            table_description: 表格描述
            table_metadata: 表格元数据（已废弃）

        Returns:
            详细的工具描述字符串，用于大语言模型理解工具用途和使用方式
        """
        # 构建项目基础描述
        project_name_str = f'表格所属项目名称："{project_name}"'
        ...
        # 构建表格基础描述
        table_info = f'表格名称："{table_name}; "'
        if table_description:
            table_info += f" 表格描述：{table_description};"

        base_description = f"""这是一个用于管理表格的工具。表格本质上是一个JSON对象，以键值对（key-value）的形式存储数据。表格的信息如下：

{project_name_str}; {project_description_str}; {project_metadata_str}

{table_info}

重要提示：表格是一个JSON对象结构，其中：
- key（键）：字符串类型，用于唯一标识数据项
- value（值）：可以是任意JSON类型（对象、数组、字符串、数字、布尔值等）

使用流程：
1. 首次使用建议通过 query_table(schema="1") 工具获取表格的结构。
...
3. 获取所有数据：不传任何参数时，返回整个表格的完整内容，除非用户特别需要，否则尽可能使用JMESPath的查询方式。
...
- 无参数模式：返回完整的JSON对象，包含表格中所有的键值对（key-value pairs）。如果表格为空，将返回空的JSON对象 {{}}

重要提示：JMESPath查询是基于表格数据进行的。表格数据可能是JSON对象（字典）或JSON数组，需要根据实际数据结构选择合适的查询语法。
...
常见错误和正确示例：
- ❌ 错误：`[?company=='puppyagent'].company_info.departments[*]` - 如果表格数据是对象，不能对对象使用`[?...]`过滤
- ✅ 正确（对象）：`company_info.departments[*]` - 直接访问对象的字段
- ✅ 正确（数组）：`[?company=='puppyagent'].company_info.departments[*]` - 如果表格数据是数组，可以先过滤再访问
- ❌ 错误：如果表格数据是对象，使用`[?name=='Alice']`会失败
- ✅ 正确（对象）：`departments[*].employees[?name=='Alice']` - 在对象的数组字段中过滤
- ✅ 正确（数组）：`[?name=='Alice']` - 如果表格数据是数组，可以直接过滤
...
使用场景：
- 首次访问表格时，或者要实现增加、修改和删除操作时，必须先调用此工具了解数据结构（可以使用 schema="1" 快速查看结构）
...
功能：在表格中批量创建新的键值对（key-value pairs）。
...
注意事项：
- 必须先使用 query_table 工具获取当前表格内容，了解现有数据结构
- key必须是字符串类型，且在当前表格中唯一
- 如果key已存在，创建操作将失败
...
示例场景：
- 用户说"在某某项目的某某表格中添加一个新的用户信息，key是user_001" → 使用此工具创建
- 用户说"在某某项目的某某表格中记录这个文档，key是doc_001" → 使用此工具创建
- 用户说"在某某项目的某某表格中保存这个配置，key是config_001" → 使用此工具创建""",
...
- updates (List[Dict]): 要更新的元素数组，每个元素是一个字典，包含：
  - key (str): 字符串类型，要更新的数据项的键名。必须已存在于表格中
  - value (Any): 任意JSON类型，新的值内容，将完全替换原有的value

使用场景：
- 当用户需要修改表格中现有数据项的值时或者需要更正或替换已有数据时
- 当需要批量更新多个键值对时

注意事项：
- 必须先使用 query_table 工具获取当前表格内容，确认要更新的key是否存在
- key必须已存在于表格中，如果不存在则更新失败
- value将完全替换原有的值，不是部分更新
- 支持批量更新，可以一次更新多个键值对

示例：
假设当前表格为：{{"user_001": {{"name": "张三", "age": 25}}}}
要更新user_001的age为26：
updates = [{{"key": "user_001", "value": {{"name": "张三", "age": 26}}}}]
执行后将得到：{{"user_001": {{"name": "张三", "age": 26}}}}

示例场景：
- 用户说"在某某项目的某某表格中修改key为user_001的用户信息" → 使用此工具更新
- 用户说"在某某项目的某某表格中更新key为doc_001的文档内容" → 使用此工具更新
- 用户说"在某某项目的某某表格中把key为config_001的配置改为新值" → 使用此工具更新""",
...
注意事项：
- 必须先使用 query_table 工具获取当前表格内容，确认要删除的key是否存在
...
示例：
假设当前表格为：{{"user_001": {{"name": "张三"}}, "user_002": {{"name": "李四"}}, "user_003": {{"name": "王五"}}}}
...
功能：获取表格数据的轻量级预览版本，只返回配置的关键字段，方便快速浏览和检索。

参数说明：
- 无参数

工具逻辑：
- 如果表格数据是 List[Dict] 类型，会对每个字典过滤掉非预览字段，只保留预览字段
- 如果预览字段列表为空，则返回所有字段
- 如果表格数据不是 List[Dict] 类型，会返回提示消息，建议使用 query_table 工具

使用场景：
- 当表格数据量较大，需要快速浏览数据时
- 当只需要查看关键字段，不需要完整数据时
- 作为 select_tables 工具的前置步骤，先预览后精选
...
3. 精确获取：调用 select_tables(field, keys) 获取完整数据""",
            "select": f"""{base_description}

功能：根据指定字段和键值列表，从表格中批量获取完整的数据记录。
...
工具逻辑：
- 如果表格数据是 List[Dict] 类型，会根据指定的 field 字段，找出该字段值在 keys 列表中的所有记录
- 返回完整的字典记录（不是轻量级版本）
- 使用精确匹配（==）进行比较
- 如果表格数据不是 List[Dict] 类型，会返回提示消息，建议使用 query_table 工具
...
调用 select_tables(field="id", keys=["1", "3"])，返回：
[
  {{"id": "1", "name": "张三", "age": 25, "address": "北京", "email": "zhang@example.com"}},
  {{"id": "3", "name": "王五", "age": 28, "address": "广州", "email": "wang@example.com"}}
]
...
2. 根据预览结果，使用 select_tables(field, keys) 获取目标记录的完整数据
3. 对获取的完整数据进行处理或展示""",
        }
    ...
    def query_table(
        ...
        Args:
            table_info: 表格信息字典，包含 table, table_id, json_pointer 等
        ...
        """
        try:
            table_id_str = table_info.get("table_id")
            json_pointer = table_info.get("json_pointer", "")

            if not table_id_str:
                return {"error": "表格ID不存在", "table_id": table_id_str}

            # 转换 table_id 为整数
            try:
                table_id = int(table_id_str)
            except (ValueError, TypeError):
                return {"error": f"无效的table_id: {table_id_str}"}
            ...
            if schema_int == 1:
                try:
                    data = table_service.get_context_structure(
                        table_id, json_pointer
                    )
                    return {
                        "message": "获取表格结构成功",
                        "data": data,
            ...
            if query:
                try:
                    data = table_service.query_context_data_with_jmespath(
                        table_id, json_pointer, query
                    )
                    ...
                    return {"error": str(e) or "JMESPath 查询失败", "query": query}
                except BusinessException as e:
                    return {"error": str(e) or "JMESPath 查询失败", "query": query}

            # 默认情况：返回所有数据
            try:
                data = table_service.get_context_data(table_id, json_pointer)
            ...
            log_error(f"Error getting table: {e}")
            return {"error": f"获取表格内容失败: {str(e)}"}

    def create_element(
        self, elements: List[Dict[str, Any]], table_info: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        批量创建元素到指定路径下
        ...
        try:
            ...
            for element in elements:
                if not isinstance(element, dict):
                    failed_keys.append(
                        {"element": element, "reason": "元素必须是字典类型"}
                    )
                    continue
    ...
    def update_element(
        self, updates: List[Dict[str, Any]], table_info: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        批量更新指定路径下的元素
        ...
        try:
            ...
            for update_item in updates:
                if not isinstance(update_item, dict):
                    failed_keys.append(
                        {"update": update_item, "reason": "更新项必须是字典类型"}
                    )
                    continue
    ...
    def delete_element(
        self, keys: List[str], table_info: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        批量删除指定路径下的元素
    ...
    def preview_data(self, table_info: Dict[str, Any]) -> Dict[str, Any]:
        """
        获取表格数据的轻量级预览版本

        Args:
            table_info: 表格信息字典，包含 table, table_id, json_pointer, preview_keys 等
        ...
        try:
            ...
            try:
                ...
                return {"error": f"无效的table_id: {table_id_str}"}
            json_pointer = table_info.get("json_pointer", "")
            preview_keys = table_info.get("preview_keys", [])
            ...
            if not isinstance(data, list):
                return {
                    "message": "当前数据不是列表类型，无法使用预览功能。请使用 query_table 工具查询数据。",
            ...
            if data and not all(isinstance(item, dict) for item in data):
                return {
                    "message": "当前数据不是 List[Dict] 类型，无法使用预览功能。请使用 query_table 工具查询数据。",
            ...
            for item in data:
                filtered_item = {
                    key: item.get(key) for key in preview_keys if key in item
                }
    ...
    def select_tables(
        self, field: str, keys: List[str], table_info: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        根据字段和键值列表批量获取完整数据
        ...
        try:
            ...
            if not isinstance(data, list):
                return {
                    "message": "当前数据不是列表类型，无法使用选择功能。请使用 query_table 工具查询数据。",
                    "data_type": str(type(data).__name__),
                }
            ...
            if data and not all(isinstance(item, dict) for item in data):
                return {
                    "message": "当前数据不是 List[Dict] 类型，无法使用选择功能。请使用 query_table 工具查询数据。",
                    "data_type": "List[mixed]",
                }
            ...
            log_error(f"Error selecting tables: {e}")
