...
class HttpJwtTokenAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def _validate_and_get_params(self, token: str, request: Request) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if instance.port != request_port:
                log_error(f"Port mismatch: instance port={instance.port}, request port={request_port}, api_key={token[:20]}...")
                return None
            ...
            log_info(f"Token validated successfully for user_id: {payload.user_id}, project_id: {payload.project_id}, context_id: {payload.context_id}, port: {request_port}")
