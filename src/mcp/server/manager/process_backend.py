    ...
    async def start_instance(
        ...
        try:
            ...
            try:
                ...
                use_uv = True
                log_info(
                    f"Using uv to run MCP server (uv version: {result.stdout.decode().strip()})"
                )
            except (
            ...
            if use_uv:
                ...
                cmd = ["uv", "run", "python", "-m", module_path] + base_cmd
                command_str = f"uv run python -m {module_path} {' '.join(base_cmd)}"
            else:
                # 使用系统 Python 解释器，以模块方式运行
                cmd = [sys.executable, "-m", module_path] + base_cmd
            ...
            try:
                ...
                if returncode is not None:
                    ...
                    log_error(
                        f"MCP instance {instance_id} failed to start: {error_msg}"
                    )
            ...
            log_info(
                f"MCP instance {instance_id} started successfully with PID {proc.pid} on port {port}"
            )
