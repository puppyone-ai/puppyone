...
def create_user_context(
    ...
    # 2. 创建用户知识库
    user_context = user_context_service.create(
        payload.user_id,
        payload.project_id,
        payload.context_name,
        payload.context_description,
...
def update_user_context(
    ...
    context_data = (
        payload.context_data
        if payload.context_data is not None and payload.context_data != {}
        else None
    )
    user_context = user_context_service.update(
        context_id,
        payload.context_name,
        payload.context_description,
        context_data,
...
def get_context_data(
    ...
    json_pointer_path: Optional[str] = Query(
        default="",
        description='JSON指针路径，使用RFC 6901标准格式。例如：/users 或 /users/123。根路径使用空字符串 "" 可以获取整个 context_data。如果不传此参数，默认为空字符串（根路径）',
        min_length=0,
    ...
    data = user_context_service.get_context_data(
        context_id=context_id, json_pointer_path=json_pointer_path
    )
