...
import type { ModelConnection, ModelConnectionCandidate } from "../../../../shared/model-connections/types";
...
export function ModelConnectionsSettings({ embedded = false, store: suppliedStore }: { embedded?: boolean; store?: ModelConnectionStore }) {
  ...
  return <section className={embedded ? "model-connections model-connections-embedded" : "desktop-utility-view desktop-settings-view model-connections"}>
    <div className={embedded ? "model-connections-content" : "desktop-utility-body desktop-settings-body model-connections-content"} data-po-scrollbar="content">
      <header><h2>{t("settings.modelConnections.title")}</h2><p>{t("settings.modelConnections.detail")}</p></header>
      ...
      <div className="model-connection-actions">
        ...
        <button type="button" disabled={state.pending.discover} onClick={() => void store.discover()}>{t(state.pending.discover ? "settings.modelConnections.discovering" : "settings.modelConnections.discover")}</button>
      </div>
      <p className="model-connection-hint">{t("settings.modelConnections.discoveryHelp")}</p>
      {state.candidates.length > 0 && <ul className="model-connection-candidates">
      ...
      {editor && <ModelConnectionSetup key={editor.connection?.id ?? editor.candidate?.baseUrl ?? "new"}
        {...editor} busy={Boolean(state.pending.save)} onSave={store.save} onCancel={() => setEditor(null)} />}
      ...
      <aside className="model-connection-managed"><h3>{t("settings.modelConnections.managed")}</h3><p>{t("settings.modelConnections.managedUnavailable")}</p></aside>
    </div>
  </section>;
}
