...
import { gmailCallback } from '@/lib/oauthApi';
...
function GmailCallbackContent() {
  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      try {
        const result = await gmailCallback(code, state);

        if (result.success) {
          setStatus('success');
          setMessage(result.message || 'Successfully connected to Gmail!');
          setTimeout(() => window.close(), 2000);
        } else {
          setStatus('error');
          setMessage(result.message || 'Failed to connect to Gmail');
          setTimeout(() => window.close(), 3000);
        }
      } catch (err) {
        setStatus('error');
