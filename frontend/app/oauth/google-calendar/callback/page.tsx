...
import { googleCalendarCallback } from '@/lib/oauthApi';
  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      try {
        ...
        if (result.success) {
          setStatus('success');
          setMessage(result.message || 'Successfully connected to Google Calendar!');
          setTimeout(() => window.close(), 2000);
        } else {
          setStatus('error');
          setMessage(result.message || 'Failed to connect to Google Calendar');
          setTimeout(() => window.close(), 3000);
        }
      } catch (err) {
        setStatus('error');
