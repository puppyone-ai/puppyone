...
import { useOnboardingStore, TrackedFile, ETLTaskState } from '../../../components/onboarding/store';
...
// --- Icons & Assets ---
const IconImg = ({ src, alt, size = 16 }: { src: string; alt: string, size?: number }) => (
  <img src={src} alt={alt} style={{ width: size, height: size, objectFit: 'contain', opacity: 0.8 }} />
);

const SOURCE_INFO: Record<string, { label: string; icon: React.ReactNode }> = {
  notion: { label: 'Notion', icon: <IconImg src="/icons/notion.png" alt="Notion" /> },
  gdocs: { label: 'Google Docs', icon: <IconImg src="/icons/Google_Docs_logo.png" alt="Google Docs" /> },
};
...
const CheckIcon = ({ size = 14 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="#4ADE80" strokeWidth="2.5">
...
export default function Step2Page() {
  ...
  const { 
    ...
    tableId, setTableId,
    projectName,
    trackedFiles, setTrackedFiles, updateTrackedFile,
    connectedApps, setConnectedApps,
    enteredUrls, setEnteredUrls,
  } = useOnboardingStore();
  ...
  const pollTaskStatus = useCallback(async () => {
    ...
    const pendingFiles = trackedFiles.filter(f => 
      f.taskId && (f.state === 'pending' || f.state === 'parsing')
    );
    ...
    try {
      ...
      result.tasks.forEach(task => {
        const fileIndex = trackedFiles.findIndex(f => f.taskId === task.task_id);
        ...
        if (task.status === 'completed') {
          ...
          newState = 'failed';
        } else if (task.status === 'mineru_parsing' || task.status === 'llm_processing') {
          newState = 'parsing';
        } else {
          newState = 'pending';
        }
  ...
  useEffect(() => {
    const hasPendingTasks = trackedFiles.some(f => 
      f.state === 'pending' || f.state === 'parsing' || f.state === 'uploading'
    );
  ...
  const hasCompletedData = completedFiles.length > 0 || enteredUrls.length > 0 || connectedApps.length > 0;
  const allFilesProcessed = trackedFiles.length > 0 && trackedFiles.every(f => f.state === 'completed' || f.state === 'failed');
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              background: isDragging ? 'rgba(69, 153, 223, 0.05)' : 'rgba(255,255,255,0.02)',
              cursor: 'pointer',
              transition: 'all 0.2s',
            }}
          >
            ...
            {trackedFiles.length === 0 ? (
              <div style={{ 
                ...
                <div style={{ fontSize: 12, color: '#666', fontWeight: 400 }}>PDF, Markdown, CSV, Text</div>
              </div>
            ) : (
                ...
                {trackedFiles.map((tracked, i) => (
                  <div key={i} style={{ 
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                      <span style={{ fontSize: 11, color: getStatusColor(tracked.state) }}>
                        {getStatusLabel(tracked.state)}
                      </span>
                      {(tracked.state === 'completed' || tracked.state === 'failed') && (
                        <div 
                          onClick={(e) => { e.stopPropagation(); removeFile(i); }} 
                          style={{ cursor: 'pointer', color: '#555', padding: '0 2px' }}
                        >
                          ×
                        </div>
                      )}
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    gap: 6,
                    padding: '6px 10px',
                    background: isConnected ? 'rgba(74, 222, 128, 0.1)' : 'rgba(255,255,255,0.03)',
                ...
                >
                  {info.icon}
                  <span style={{ fontSize: 12, color: isConnected ? '#4ADE80' : '#ccc' }}>
                    {info.label}
                  </span>
            ...
            <div
              ...
              style={{
                ...
                gap: 4,
                padding: '6px 10px',
                background: (showUrlInput || enteredUrls.length > 0) ? 'rgba(69, 153, 223, 0.1)' : 'rgba(255,255,255,0.03)',
            ...
            >
              <span style={{ fontSize: 12, color: (showUrlInput || enteredUrls.length > 0) ? '#4599DF' : '#ccc' }}>
                {enteredUrls.length > 0 ? `URL (${enteredUrls.length})` : '+ URL'}
              </span>
            </div>
          </div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
                    {enteredUrls.map((url, i) => (
                      <div key={i} style={{ 
                        ...
                        <div onClick={() => removeUrl(i)} style={{ cursor: 'pointer', color: 'rgba(255,255,255,0.4)', paddingLeft: 4 }}>×</div>
                      </div>
                    ))}
                  </div>
                )}
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 16 }}>
          <div
            ...
            style={{
              ...
              gap: 6,
              padding: '10px 28px',
              background: hasCompletedData ? '#EDEDED' : 'rgba(255,255,255,0.05)',
