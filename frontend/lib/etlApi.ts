...
export type ETLStatus = 'pending' | 'mineru_parsing' | 'llm_processing' | 'completed' | 'failed' | 'cancelled'
...
export function getStatusDisplayText(status: ETLStatus | 'uploading'): string {
  switch (status) {
    ...
    case 'mineru_parsing': return '正在识别文档...'
    case 'llm_processing': return '正在提取结构化信息...'
...
export async function uploadAndSubmit(
  ...
  const response = await fetch(
    `${API_URL}/api/v1/etl/upload_and_submit`,
    {
      method: 'POST',
      headers: {
...
export async function getETLTaskStatus(
  ...
  const response = await fetch(
    `${API_URL}/api/v1/etl/tasks/${taskId}`,
    {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${accessToken}`
