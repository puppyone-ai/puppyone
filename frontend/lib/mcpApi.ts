 ...
 * 更新 Tool
 */
export async function updateTool(toolId: number, request: ToolUpdateRequest): Promise<Tool> {
 ...
 * 获取当前用户的所有 MCP v2 实例
 */
export async function getMcpV2Instances(skip = 0, limit = 100): Promise<McpV2Instance[]> {
...
export async function createMcpV2WithBindings(
  ...
  return post<McpV2CreateWithBindingsResponse>('/api/v1/mcp/with_bindings', request)
}

/**
 * 更新 MCP v2 实例
 */
export async function updateMcpV2(apiKey: string, request: McpV2UpdateRequest): Promise<McpV2Instance> {
 ...
 * 批量绑定 Tool 到 MCP v2
 */
export async function createBindings(apiKey: string, bindings: BindingRequest[]): Promise<void> {
 ...
 * 更新绑定状态（启用/禁用 Tool）
 */
export async function updateBinding(apiKey: string, toolId: number, status: boolean): Promise<void> {
 ...
 * 解绑 Tool
 */
export async function deleteBinding(apiKey: string, toolId: number): Promise<void> {
...
export async function createToolsAndMcp(params: {
  ...
  const { tableId, jsonPath = '', permissions, toolNamePrefix = '', mcpName, customDefinitions } = params
