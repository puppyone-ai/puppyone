...
interface UseMentionReturn {
  ...
  handleInputChange: (e: React.ChangeEvent<HTMLTextAreaElement>, currentValue: string, setValue: (v: string) => void) => void;
  handleSelectMention: (key: string, inputValue: string, setInputValue: (v: string) => void, inputRef: { focus: () => void; setSelectionRange: (start: number, end: number) => void } | null) => void;
...
export function useMention({ data }: UseMentionOptions): UseMentionReturn {
  ...
  const mentionInfo = useMemo(() => {
    if (mentionStartPos === null) return { basePath: '', query: '', keys: [] as string[] };
  ...
  const handleSelectMention = useCallback((
    ...
    inputRef: { focus: () => void; setSelectionRange: (start: number, end: number) => void } | null
  ) => {
    ...
    const hasChildren = selectedValue && typeof selectedValue === 'object' && 
      (Array.isArray(selectedValue) ? selectedValue.length > 0 : Object.keys(selectedValue).length > 0);
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent, onSend: () => void): boolean => {
    // 处理补全菜单的键盘导航
