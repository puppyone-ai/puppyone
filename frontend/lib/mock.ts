  ...
  const rows = Array.from({ length: 10 }, (_, idx) => {
    ...
    columns.forEach((col, colIdx) => {
      if (col.includes('uuid')) {
        record[col] =
          `user-${((base * (colIdx + 3)) % 9).toString(16)}${base.toString(16).padStart(4, '0')}`;
        ...
        col.includes('effective_at')
      ) {
        record[col] =
          `2025-11-${String((base % 9) + 1).padStart(2, '0')} ${String(9 + colIdx).padStart(2, '0')}:${String(
            (base * 7) % 60
          ).padStart(2, '0')}`;
      } else if (col.includes('endpoint')) {
        record[col] =
          `/api/v${(base % 5) + 1}/${['users', 'billing', 'shipping'][colIdx % 3]}`;
