...
export function BackgroundTaskNotifier() {
  ...
  useEffect(() => {
    ...
    return () => { delete (window as any).clearETLTasks }
  }, [])
  ...
  const checkTaskStatus = useCallback(async () => {
    ...
    try {
      ...
      const updatedTasks = pendingTasks.map(pendingTask => {
        ...
        const isTerminal = pendingTask.status === 'completed' || pendingTask.status === 'failed' || pendingTask.status === 'cancelled'
          ...
          if (isTerminalStatus(task.status)) {
            if (task.status === 'completed') {
              window.dispatchEvent(new CustomEvent('etl-task-completed', { 
                detail: { taskId: task.task_id, filename: pendingTask.filename, tableId: pendingTask.tableId } 
              }))
...
export function addPendingTasks(tasks: Omit<PendingTask, 'timestamp'>[]) {
  const existing = JSON.parse(sessionStorage.getItem(STORAGE_KEY) || '[]') as PendingTask[]
  const newTasks = tasks.map(t => ({ ...t, timestamp: Date.now() }))
  sessionStorage.setItem(STORAGE_KEY, JSON.stringify([...existing, ...newTasks]))
  window.dispatchEvent(new CustomEvent('etl-tasks-updated'))
}
...
export function replacePlaceholderTasks(tableId: string, realTasks: Omit<PendingTask, 'timestamp'>[]) {
  ...
  sessionStorage.setItem(STORAGE_KEY, JSON.stringify([...filtered, ...newTasks]))
  window.dispatchEvent(new CustomEvent('etl-tasks-updated'))
}
