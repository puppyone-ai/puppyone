...
const VirtualRow = React.memo(function VirtualRow({
  ...
  const renderPrimitiveValue = () => {
    ...
    if (
      ...
      'status' in node.value &&
      (node.value.status === 'pending' || node.value.status === 'failed')
    ) {
      const isPending = node.value.status === 'pending'
      const message = String(node.value.message || (isPending ? 'Processing...' : 'Failed'))
