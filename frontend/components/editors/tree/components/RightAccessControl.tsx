...
export function RightAccessControl({
  ...
  const handleToggle = useCallback(
    (toolId: string, enabled: boolean) => {
      ...
      onAccessChange?.(path, { ...currentTools, [toolId]: enabled } as McpToolPermissions);
    },
  ...
  return (
    <div
      ...
      style={{
        ...
        flexShrink: 0,
        // 关键：整体宽度根据状态变化
        width: isExpanded ? (26 + 8 + MENU_WIDTH) : (isAnyExpanded ? (26 + 8 + MENU_WIDTH) : 'auto'),
    ...
    >
      ...
      <div
        style={{
          ...
          opacity: isConfigured || isActive || isExpanded || hovered || isAnyExpanded ? 1 : 0,
          background: isExpanded ? 'rgba(249, 115, 22, 0.2)' : (hovered ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.1)'),
          border: isExpanded ? '1px solid rgba(249, 115, 22, 0.3)' : '1px solid transparent',
        }}
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
        onClick={e => { e.stopPropagation(); onExpandChange(!isExpanded); }}
        ...
        {isConfigured ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'center', width: '100%' }}>
            {(configuredAccess as any)?.['shell_access'] && (
              <div style={{ color: '#fb923c', display: 'flex', alignItems: 'center', justifyContent: 'center', width: 14, height: 14 }}>
            ...
            {MCP_TOOLS.filter(t => (configuredAccess as any)?.[t.id]).map(tool => (
              <div key={tool.id} style={{ color: '#fb923c', display: 'flex', alignItems: 'center', justifyContent: 'center', width: 14, height: 14 }} title={tool.label}>
                {TOOL_ICONS[tool.id]}
              </div>
            ))}
          </div>
        ) : (
          <div style={{ color: isExpanded ? '#f97316' : (hovered ? '#e2e8f0' : '#6b7280'), display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {DEFAULT_TOOL_ICON}
          </div>
        )}
      </div>
      ...
      >
        {isExpanded && (
          ...
          >
            {/* Header */}
            <div style={{ display: 'flex', alignItems: 'center', height: 28, padding: '0 4px 0 6px', marginBottom: 4 }}>
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: '#e2e8f0', whiteSpace: 'nowrap' }}>{getDisplayName()}</span>
                <span style={{ fontSize: 11, color: '#71717a', fontFamily: 'monospace', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', opacity: 0.5 }}>{path || '/'}</span>
              </div>
            ...
            <div style={{ paddingLeft: 8, display: 'flex', flexDirection: 'column', gap: 1 }}>
              {[{ id: 'shell_access', label: 'Bash' }, ...MCP_TOOLS].map(tool => {
                ...
                return (
                  <div key={tool.id}>
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, width: 52, flexShrink: 0, color: '#e2e8f0' }}>
                        <div style={{ width: 14, height: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', color: isBash ? 'inherit' : (TOOL_TYPE_CONFIG[tool.id]?.color || 'inherit') }}>
                          {isBash ? (
                            <svg width='14' height='14' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2'>
                              <polyline points='4 17 10 11 4 5' /><line x1='12' y1='19' x2='20' y2='19' />
                            </svg>
                        ...
                        <span style={{ fontSize: 12, fontWeight: 500, whiteSpace: 'nowrap' }}>{tool.label}</span>
                      </div>
                      <div style={{ flex: 1, fontSize: 12, color: '#9ca3af', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{config.name}</div>
                      <div
            ...
            <div style={{ padding: '8px 8px 4px' }}>
              <button
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', background: 'rgba(255,255,255,0.03)', border: '1px dashed rgba(255,255,255,0.1)', borderRadius: 6, color: '#71717a', fontSize: 11, cursor: 'pointer', width: '100%', justifyContent: 'center', transition: 'all 0.15s' }}
                onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.06)'; e.currentTarget.style.color = '#a1a1aa'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.2)'; }}
                onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.03)'; e.currentTarget.style.color = '#71717a'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)'; }}
              >
                <svg width='10' height='10' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2'><path d='M12 5v14M5 12h14' strokeLinecap='round' strokeLinejoin='round' /></svg>
                Add Capability
              </button>
            </div>
            ...
            {isConfigured && (
              <div style={{ padding: '4px 8px 4px' }}>
                <button
                  onClick={() => { onRemove?.(path); onExpandChange(false); }}
