...
const MenuPanel = React.memo(function MenuPanel({
  ...
  // Bash 模式状态
  const isBashReadOnly =
    (configuredAccess as any)?.['shell_access_readonly'] || false;
      ...
      <div key={tool.id}>
        <div
          style={{
            ...
            gap: 8,
            borderRadius: 6,
            background: isToolExpanded ? '#2C2C2C' : 'transparent',
            opacity: isEnabled ? 1 : 0.6,
            cursor: 'default',
            transition: 'background 0.1s',
          }}
          onMouseEnter={e => {
            if (!isToolExpanded) e.currentTarget.style.background = '#2C2C2C';
          }}
          onMouseLeave={e => {
            if (!isToolExpanded)
          ...
          >
            <div>
              ...
              <input
                ...
                value={config.name}
                onChange={e =>
                  updateToolConfig(tool.id, 'name', e.target.value)
                }
                style={{
                  width: '100%',
            ...
            <div>
              ...
              <textarea
                value={config.desc}
                onChange={e =>
                  updateToolConfig(tool.id, 'desc', e.target.value)
                }
                rows={2}
    ...
    >
      ...
      <div style={{ paddingLeft: 8 }}>
        ...
        >
          ...
          <span
            style={{ flex: 1, fontSize: 12, fontWeight: 500, color: '#e2e8f0' }}
          >
            Security (NLS)
          </span>
          <span
...
const RowButton = React.memo(function RowButton({
  ...
  const hasBash =
    ...
    !!(configuredAccess as any)?.['shell_access_readonly'];
  // 计算 Tools 数量时排除 Bash
        ...
        <>
          ...
          {showBash ? (
            <BashButton
              ...
              localHovered={localHovered}
              title='Shell Access Enabled'
              showBackground={localHovered}
            >
              <svg
                width='12'
                height='12'
                ...
                fill='none'
                stroke={
                  isExpanded ? '#fff' : localHovered ? '#f97316' : '#ffa73d'
                }
                strokeWidth='2'
                strokeLinecap='round'
                strokeLinejoin='round'
          ...
          {showTools && (
            <ToolsButton
              isExpanded={isExpanded}
              localHovered={localHovered}
              title={`${otherToolsCount} Tools Enabled`}
              showBackground={localHovered}
            >
              <svg
                width='12'
...
export function RightAccessSidebar({
  ...
  const isOverlayMode =
    (containerWidth || 0) < OVERLAY_THRESHOLD && (containerWidth || 0) > 0;

  // Menu 区域的虚拟 hover 路径（当鼠标在 menu 区域移动时计算）
  const [menuAreaHoveredPath, setMenuAreaHoveredPath] = useState<string | null>(
    null
  );
  // 按钮列区域的虚拟 hover 路径
  const [buttonAreaHoveredPath, setButtonAreaHoveredPath] = useState<
    string | null
  >(null);
  ...
  const handleMenuAreaMouseMove = useCallback(
    (e: React.MouseEvent) => {
  ...
  const handleButtonAreaMouseMove = useCallback(
    (e: React.MouseEvent) => {
  ...
  return (
    <div
      ...
      style={{
        ...
        width:
          hasExpandedPopover && !isOverlayMode ? 54 + 8 + 300 + 24 : 54 + 24,
        ...
        {visibleRows.map(({ node, offsetY }) => (
          ...
          >
            <RowButton
              ...
              isHovered={
                hoveredRowPath === node.path ||
                menuAreaHoveredPath === node.path ||
                buttonAreaHoveredPath === node.path
              }
        ...
        {hasExpandedPopover && expandedRowInfo && (
          ...
          >
            <MenuPanel
              ...
              configuredAccess={
                configuredAccessMap.get(lockedPopoverPath!) || null
              }
