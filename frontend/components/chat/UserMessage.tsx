...
export default function UserMessage({ message, showAvatar = true, showBorder = true, isTyping = false }: UserMessageProps) {
  ...
  const styles: { [key: string]: CSSProperties } = {
    ...
    bubble: { padding: '4px 10px', borderRadius: '12px', boxShadow: 'none', position: 'relative', background: '#1e1e1e', color: '#dcdcdc', border: 'none', cursor: 'default' },
    content: { fontSize: '13px', whiteSpace: 'normal', lineHeight: '1.5', margin: 0, textAlign: 'left' },
        ...
        {!isTyping && (
          <div style={styles.metaBar}>
            <div style={styles.timestamp}>
              {message.timestamp ? message.timestamp.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }) : ''}
            </div>
