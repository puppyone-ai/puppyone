...
import { isTerminalStatus, getStatusDisplayText, ETLTaskStatus } from '../lib/etlApi'
import { getAllPendingTasks, clearAllTasks, PendingTask } from './BackgroundTaskNotifier'
...
export function TaskStatusWidget() {
  ...
  const loadTasks = useCallback(() => {
    ...
    // 有新任务时自动展开
    if (tasksWithStatus.length > 0 && tasksWithStatus.some(t => !isTaskTerminal(t.displayStatus))) {
      setIsExpanded(true)
    }
  }, [])
  ...
  const processingCount = tasks.filter(t => !isTaskTerminal(t.displayStatus)).length
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              <button 
                ...
                style={{ background: 'none', border: 'none', padding: 4, cursor: 'pointer', color: '#6b7280', display: 'flex' }}
                title="收起"
              >
