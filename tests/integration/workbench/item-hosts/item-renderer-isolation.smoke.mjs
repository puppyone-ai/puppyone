...
import { createFixtureAgentRuntime } from "../../../fixtures/agent/runtimes/item-agent-runtime.mjs";

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../../..");
const fixture = await fs.mkdtemp(path.join(os.tmpdir(), "puppyone-item-renderer-smoke-"));
const output = path.join(root, "artifacts", "tests", "workbench", "item-isolation");
...
async function run() {
  ...
  const agentService = createAgentProcessService({ utilityProcess, modulePath: path.join(root, "tests/fixtures/agent/runtimes/item-agent-utility.mjs"),
