...
it("starts one native session for an ordinary drag and reports failed admission", async () => {
  const root = createRoot(document.createElement("div"));
  const failure = vi.fn();
  const startResourceDrag = vi.fn(async () => true);
  window.puppyoneDesktop = { startResourceDrag, resourceDragSessionSupported: true } as unknown as NonNullable<typeof window.puppyoneDesktop>;
