...
describe("project-owned Terminal runtime pool", () => {
  it("keeps each screen and captured root through view unmounts", () => {
    ...
    const entry = pool.ensure("terminal-a", "shell");
    screen.unmount(document.createElement("div"));
    expect(pool.ensure("terminal-a", "shell")).toBe(entry);
    expect(factory).toHaveBeenCalledOnce();
    expect(factory).toHaveBeenCalledWith(expect.objectContaining({ workspacePath: "/projects/a", projectContext: context("a") }));
  ...
  it("refuses runtime allocation after the project closes", () => {
    ...
    expect(() => pool.ensure("terminal-a", "shell")).toThrow();
