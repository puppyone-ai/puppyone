...
utilityProcess.fork = (modulePath, args, options) => {
  ...
  utilities.set(options.serviceName, child);
  return child;
};
...
ipcMain.handle = (channel, listener) => handle(channel, async (event, ...args) => {
  ...
  if (channel === "terminal:create" && result?.pid && !terminals.some((record) => record.pid === result.pid)) {
    const request = args[0];
    const record = { pid: result.pid, utilityPid: utilities.get(`PuppyOne terminal ${result.id}`)?.pid, rendererPid: event.sender.getOSProcessId(), cwd: request.projectContext.rootPath,
...
try {
  ...
  const a = terminals[0];
  await untilPresented(a, "A DOM presentation");
  assert(new Set([process.pid, window.webContents.getOSProcessId(), a.utilityPid, a.pid]).size === 4 && a.rendererPid === window.webContents.getOSProcessId(), "Terminal execution must be isolated while UI shares the window renderer");
  ...
  await untilRenderer(`!document.querySelector('[data-terminal-session-host-id="${a.receipt.id}"]')`, "detach A DOM presentation");
  ...
  // The retained project runtime continues receiving output while its DOM is detached.
  await click(".desktop-terminal-launcher-shell");
  ...
  await untilPresented(a, "restore A DOM presentation");
  await untilRenderer(`!document.querySelector('[data-terminal-session-host-id="${b.receipt.id}"]')`, "detach B DOM presentation");
  ...
  try {
    ...
    await untilPresented(a, "DOM presentation after split");
    await click(`[data-terminal-tab-session-id="${a.receipt.id}"] .desktop-terminal-tab-select`);
    await untilRenderer(`document.querySelector('[data-terminal-session-host-id="${a.receipt.id}"] .xterm-helper-textarea') === document.activeElement`, "terminal tab focuses its input");
    await evaluate("document.querySelector('.desktop-terminal-new-button').focus()");
    ...
    assert(await evaluate("document.activeElement.matches('.desktop-terminal-new-button')"), "Content summary stole DOM focus");
    ...
    await untilPresented(a, "DOM presentation after tab movement");
    assert(window.contentView.children.length === 0, "Moving a DOM tab allocated a native view");
  ...
  await untilPresented(a, "retained DOM view after reunion");
  ...
  const report = { ok: true, temp, roots, resizeObservations, aTabs, bTabs, agentDraftVerified, blankTabsVerified: !agentDraftVerified, domPresentationRestored: true, domFocusVerified: true,
    summaryDoesNotStealFocus: true, groupGripRemoved: true, domSplitterVerified: true, tabMovementVerified: true,
...
function untilPresented(record, label) {
  return untilRenderer(`document.querySelector('[data-terminal-session-host-id="${record.receipt.id}"] .desktop-terminal-contribution-host')?.getAttribute('aria-hidden') === 'false'`, label);
}
...
async function untilTerminal(record, text, label) {
  await until(async () => !record.sender.isDestroyed()
    && await record.sender.executeJavaScript(`document.querySelector('[data-terminal-session-host-id="${record.receipt.id}"] .xterm-screen')?.textContent.includes(${JSON.stringify(text)})`), label);
}
async function agentContents() { return window.webContents; }
...
async function resizeSplit() {
  const rect = await evaluate(`(() => { const r = document.querySelector('.desktop-terminal-splitter').getBoundingClientRect(); return { x: r.x + r.width / 2, y: r.y + 60 }; })()`);
  ...
  const send = (type, x, buttons) => window.webContents.debugger.sendCommand("Input.dispatchMouseEvent", {
    type, x: Math.round(x), y: Math.round(rect.y), buttons, button: "left", clickCount: 1, pointerType: "mouse",
  });
  await send("mousePressed", rect.x, 1);
  await send("mouseMoved", rect.x - 40, 1);
  await send("mouseReleased", rect.x - 40, 0);
  await untilRenderer("!document.querySelector('.desktop-terminal-splitter[data-resizing=true]')", "splitter release");
  assert(await evaluate("document.querySelector('[data-terminal-split-id]').style.getPropertyValue('--desktop-terminal-first-track')") !== before, "DOM splitter did not commit resize");
}
