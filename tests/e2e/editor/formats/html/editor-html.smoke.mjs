...
const original = '<!DOCTYPE html>\r\n<!-- preserved -->\r\n<html><head><style>body{margin:30px}</style></head><body><h1 id="title">Original title</h1><p>Untouched paragraph</p></body></html>';
...
const frame = () => window.webContents.mainFrame.frames.find(child => child.url === "about:srcdoc");
...
app.whenReady().then(async () => {
  try {
    ...
    await click('.tree-row.file[aria-label="page.html"]');
    await until(() => evaluate("document.querySelector('.html-document-editor iframe')?.srcdoc.includes('External replacement')"), "HTML reopened");
