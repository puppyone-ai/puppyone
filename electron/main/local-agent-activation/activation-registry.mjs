...
export function createActivationRegistry({ platform = process.platform, arch = process.arch, ports = {} } = {}) {
  ...
  return new Map(setupRegistry.filter(route => route.platforms.includes(platform)).map(route => [route.id, Object.freeze({
    ...
    // Unsupported ports/platforms retain the truthful guided route.
    recipe: ports[route.id] ? recipes[route.id] ?? null : null,
    port: recipes[route.id] ? ports[route.id] ?? null : null,
  })]));
}
