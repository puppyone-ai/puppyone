...
export function parseWorkerArguments(argv) {
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    if (value === "--mode") {
      if (values[++index] !== "rpc") throw new Error(`${BUILT_IN_AGENT_DISPLAY_NAME} worker supports only Pi RPC mode.`);
...
function validSessionId(value) {
  if (typeof value !== "string" || !/^[A-Za-z0-9._-]{1,200}$/u.test(value)) {
    throw new TypeError(`${BUILT_IN_AGENT_DISPLAY_NAME} session id is invalid.`);
  }
  return value;
}
