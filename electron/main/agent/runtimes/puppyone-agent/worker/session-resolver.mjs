...
export async function resolvePuppyOneSession({ cwd, sessionDir, sessionId = null, inMemory = false }) {
  if (!path.isAbsolute(cwd) || !path.isAbsolute(sessionDir)) {
    throw new TypeError(`${WORKSPACE_AGENT_DISPLAY_NAME} session paths must be absolute.`);
  }
  ...
  if (!match) throw new Error(`${WORKSPACE_AGENT_DISPLAY_NAME} session not found: ${sessionId}`);
