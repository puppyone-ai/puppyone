...
export function providerActivityIdentity(projection, event, label) {
    ...
    for (let index = projection.activities.length - 1; index >= firstCandidate; index -= 1) {
        ...
        if ((activity.kind !== "warning" && activity.kind !== "error")
            || providerMessageFingerprint(activity.label) !== fingerprint)
            continue;
        if (turnId && activity.turnId && activity.turnId !== turnId)
            continue;
        if ((!turnId || !activity.turnId) && event.sequence - activity.sequence > 16)
            continue;
...
export function isNonDiagnosticProviderStatusMessage(value) {
 ...
 */
export function legacyProviderConnectionUpdate(event, label) {
...
function retryFraction(value) {
    const match = value.match(/\b(\d+)\s*\/\s*(\d+)\b/);
    if (!match)
        return null;
    const attempt = Number(match[1]);
    const maxAttempts = Number(match[2]);
    if (!Number.isSafeInteger(attempt) || !Number.isSafeInteger(maxAttempts) || attempt <= 0 || maxAttempts <= 0)
        return null;
