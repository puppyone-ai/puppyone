...
export function composeLocalAgentActivation({ app, discoveryPort, installationService, fetch, openExternal, publish }) {
  ...
  return createLocalAgentActivationService({
    registry: createActivationRegistry(),
    ...
    async createContext({ file, signal }) {
      ...
      const candidate = { executablePath: file, canonicalIdentity: await fs.realpath(file) };
      const env = activationEnvironment(process.env);
      ...
      return {
        async run(args, options) {
          ...
          signal.throwIfAborted();
          return runActivationProcess(executable, args, { ...options, signal, env, cwd: homedir });
        },
      };
    },
  });
