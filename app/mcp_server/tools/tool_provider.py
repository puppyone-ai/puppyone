...
from app.mcp_server.tools.context_tool import tool_types
from app.mcp_server.tools.context_tool import ContextTool
    ...
    def get_tool_description(self, tool_type: tool_types, context_info: dict) -> str:
        ...
        if tool_type in self.tool_definitions:
            ...
            try:
                ...
                log_error(f"Missing parameter in tool description template for {tool_type}: {e}")
                return template  # 如果缺少参数，返回原始模板
            except Exception as e:
                log_error(f"Error formatting tool description template for {tool_type}: {e}")
...
def create_tool_definition_provider(
    ...
    if tools_definition and len(tools_definition) > 0:
        ...
        log_info(f"Using custom tool definition provider with {len(tools_definition)} tools")
        return provider
    else:
