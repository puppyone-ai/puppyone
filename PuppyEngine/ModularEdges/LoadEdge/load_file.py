...
class FileLoadStrategy(LoadStrategy):
    ...
    def load(
        ...
        # If explicit file_configs are provided, honor them (backward compatible)
        file_configs = self.extra_configs.get("file_configs")
        if not file_configs:
            # Try to derive from self.content (prefetch produced file list)
            ...
            if not derived_files:
                raise PuppyException(1300, "No files provided for parsing")
