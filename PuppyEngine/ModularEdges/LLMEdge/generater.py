...
class ChatService:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not self.api_key:
            raise puppy_exception(3701, "Missing Large Language Model API Key")
        ...
        if not self.model:
            raise puppy_exception(3702, "Missing Large Language Model Name")

        if not messages:
            raise puppy_exception(3700, "Missing Prompt Message", "The messages field is required for the chat completion tasks with the specific LLM.")
