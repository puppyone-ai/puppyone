...
class DataLoader:
    ...
    def _load_text(
        ...
        if not text:
            raise puppy_exception(1100, "Empty Text Content")
        return text
    ...
    def _load_structured_text(
        ...
        if not structured:
            raise puppy_exception(1101, "Empty Structured Text Content")
        return structured
    ...
    def _load_weblink(
        ...
        if not url:
            raise puppy_exception(1200, "Empty Weblink")
        ...
        if mode not in mode_dict:
            raise puppy_exception(1201, "Invalid Mode", f"Invalid mode '{mode}' provided. Supported modes: {list(mode_dict.keys())}")
    ...
    def _load_file(
        ...
        if not file_path:
            raise puppy_exception(1300, "Empty File Path")
    ...
    def _load_database(
        ...
        if not db_configs:
            raise puppy_exception(1400, "Empty Database Configuration")
        extra_configs = self.data.get("extra_configs", {})
