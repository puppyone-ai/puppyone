...
class SearchPerplexity:
    @global_exception_handler(3506, "Error Searching Using Perplexity Search")
    def process(
        self,
        edge: Dict[str, Any],
        ...
        """
        Process the Perplexity search edge, execute the search operation using the specified search type.

        Returns:
            Dict[str, Dict[str, Any]]: A dictionary containing the target block IDs and their output data.
        
        
        INPUTS, FOR EXAMPLE:

        edge = {
        "search-1728381295016": {
            "type": "search",
            "data": {
              "search_type": "web",
        ...
        OUTPUTS, FOR EXAMPLE:
        {
            "2": {
                "type": "text",
                "data": {
                    "content": "To train an anime waifu using AI, follow these steps:\n
                        1. **Dataset Preparation**: Collect and organize high-quality anime images.\n\n
                        2. **Create Embedding**: Use HCP-Diffusion tools to create embeddings.\n
                           ```bash\n
                           python -m hcpdiff.tools.create_embedding <model_path> <word_name> <word_size>\n
                           ```\n\n
                        For anime physique training, combine strength and flexibility exercises. 
                        Use Lora models for AI art generation by selecting a model, installing it, 
                        and using trigger words in prompts."
                }
            }
        }
        ...
        if "query_id" in edge_data:
            block_id = next(iter(edge_data["query_id"].keys()))
            print("BLOCK ID", block_id)
            if block_id in input_blocks:
                query = input_blocks[block_id].get("data", {}).get("content", "")
...
class SearchGoogle:
    @global_exception_handler(3506, "Error Searching Using Google Search")
    def process(
        self,
        edge: Dict[str, Any],
        input_blocks: Dict[str, Dict[str, Any]]
        ...
        OUTPUTS, FOR EXAMPLE:
        {
            "2": {
                ...
                "data": {
                    "content": [
                        {
                            "title": "Dreambooth anime character training: Stable Diffusion 1.4 vs. Waifu ...",
                            "link": "https://www.reddit.com/r/StableDiffusion/comments/y5y20e/dreambooth_anime_character_training_stable/",
                            "snippet": "Oct 17, 2022 ... The Waifu Diffusion model is much better. It matches the original art style of the character while the stable diffusion model had a tendency to approximate the\xa0..."
                        },
                        {
                            "title": "deepghs/waifuc: Efficient Train Data Collector for Anime ... - GitHub",
                            "link": "https://github.com/deepghs/waifuc",
                            "snippet": "Efficient Train Data Collector for Anime Waifu. This project is still under development, official version will be released soon afterwards."
                        },
                        {
                            "title": "Looking for a course that will teach me how to make my beautiful ...",
                            "link": "https://www.reddit.com/r/blenderhelp/comments/jnmg0w/looking_for_a_course_that_will_teach_me_how_to/",
                            "snippet": "Nov 4, 2020 ... Before that I've done every single one of Blender Guru's tutorials on youtube. My main goal is to sculpt my anime waifu (or any waifu, doesn't\xa0..."
                        }
                    ]
                }
            }
        ...
        # Use the Google search implementation from WebSearchClient
        url = "https://www.googleapis.com/customsearch/v1"
        params = {
            "q": query,
            "key": os.environ.get("GCP_API_KEY"),
            "cx": os.environ.get("CSE_ID"),
            "num": edge_data.get("top_k", 5)  # Use top_k from edge data or default to 5
        }
        ...
        for item in search_results:
            formatted_results.append({"title": item.get('title', ''),
                                  "link": item.get('link', ''),
                                  "snippet": item.get('snippet', '')})

        target_block_dict = {}
        # Get the target block ID from outputs if it exists
        for key, value in edge_data.get("outputs", {}).items():
            target_block_dict[key] = {
                "type": "structured",
...
class SearchEdgeProcessor:
    ...
    def process(
        ...
        """
        Process the Search edge, execute the search operation using the specified search type.
        ...
        input_block_ids = edge_data.get("inputs", {})

        # Get query from input blocks if specified


        if "query_id" in edge_data:
            block_id = edge_data["query_id"].get("1")  # Get the first query ID
            if block_id in input_blocks:
                query = input_blocks[block_id].get("data", {}).get("content", "")
        ...
        retrieved_results = SearchClientFactory.create_search_client(
            search_type=edge_data.get("search_type", ""),
            sub_search_type=edge_data.get("sub_search_type", ""),
            query=query,
        ...
        target_block_dict = {}
        for key, value in edge_data.get("outputs", {}).items():
            target_block_dict[key] = {"data": {"content": retrieved_results}}
