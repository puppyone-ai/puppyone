...
class Chunk():
    ...
    def process(
        ...
        # Get input content from the input blocks
        input_content = ""
        edge_data = edge[list(edge.keys())[0]].get("data", {})
        ...
        for block_id, label in edge_data.get("inputs", {}).items():
            if block_id in input_blocks:
                ...
                input_content = str(block_content)
                break  # Assuming we only need one input for chunking

        chunks = ChunkingFactory.create_chunking(
            chunking_mode=edge_data.get("chunking_mode", ""),
            sub_mode=edge_data.get("sub_chunking_mode", ""),
            doc=input_content,
        ...
        # Create output dictionary matching the target blocks
        target_block_dict = {}
        for key, value in edge_data.get("outputs", {}).items():
            target_block_dict[key] = {"data": {"content": processed_output}}
